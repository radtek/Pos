...
XBilledSalesTotalsDetailsReportSection::XBilledSalesTotalsDetailsReportSection(Database::TDBTransaction* dbTransaction, TGlobalSettings* globalSettings, TDateTime* startTime, TDateTime* endTime)
	:BaseReportSection(mmXReport, mmBilledSalesTotalsDetailsSection, dbTransaction, globalSettings, startTime, endTime)
{
    dataFormatUtilities = new DataFormatUtilities;
    reportFinancialCalculations = new ReportFinancialCalculations;
}
...
void XBilledSalesTotalsDetailsReportSection::GetOutput(TPrintout* printOut, TDateTime* startTime, TDateTime* endTime)
{
    TTransactionInfo TransactionInfo;
    AnsiString deviceName = TDeviceRealTerminal::Instance().ID.Name;
