...
void TDBGroups::BuildXMLListGroups(Database::TDBTransaction &DBTransaction, TPOS_XMLBase &Data)
{
	try
	{
		...
		TiXmlElement * List = new TiXmlElement(xmlEleListGroups);
		...
		GetGroupsList(DBTransaction, Groups);

		for (std::vector <ContactGroup> ::iterator ptrGroups = Groups.begin(); ptrGroups != Groups.end(); ptrGroups++)
		{

			TiXmlElement *EleListGroups = new TiXmlElement(xmlEleGroup);
			EleListGroups->SetAttribute(xmlAttrGroupID, ptrGroups->Key);
			EleListGroups->SetAttribute(xmlAttrGroupName, ptrGroups->Name.t_str());
			EleListGroups->SetAttribute(xmlRequestType, ptrGroups->Type);
			EleListGroups->SetAttribute(xmlAttrSiteID, TGlobalSettings::Instance().SiteID);
			List->LinkEndChild(EleListGroups);
		}
...
void TDBGroups::BuildXMLListMemberGroups(Database::TDBTransaction &DBTransaction, TPOS_XMLBase &Data)
{
	try
	{
		// Update the IntaMate ID with the Invoice Number.
		...
		// Insert DOCTYPE definiation here.
		TiXmlElement *List = new TiXmlElement(xmlEleListMemberGroups);
		List->SetAttribute(xmlAttrID, AnsiString(Data.IntaMateID).c_str());
		List->SetAttribute(xmlAttrSiteID, TGlobalSettings::Instance().SiteID);
		int status;

		TIBSQL *IBInternalQuery = DBTransaction.Query(DBTransaction.AddQuery());
		IBInternalQuery->SQL->Text = "select * FROM GROUPS";
		...
		for (; !IBInternalQuery->Eof; IBInternalQuery->Next())
		{
			...
			status = IBInternalQuery->FieldByName("STATUS")->AsInteger;

			TiXmlElement *EleMemberGroup = new TiXmlElement(xmlEleGroup);
			EleMemberGroup->SetAttribute(xmlAttrGroupID, temp.Key);
			EleMemberGroup->SetAttribute(xmlAttrGroupName, temp.Name.t_str());
			EleMemberGroup->SetAttribute(xmlRequestType, temp.Type);
			EleMemberGroup->SetAttribute(xmlAttrStatus, status);
			List->LinkEndChild(EleMemberGroup);
		}
		Data.Doc.LinkEndChild(List);
	}
	catch(Exception & E)
