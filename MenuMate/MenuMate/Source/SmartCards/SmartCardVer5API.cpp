...
TSmartCardVer5::TSmartCardVer5(SCARDHANDLE inhCard,TSmartCardBlock &BlockData,TSyndCode inSyndCode)
   : TSmartCard(inhCard,BlockData,inSyndCode)
{
   VersionInfo    = SMART_CARD_VERSION_FIVE;
}

TSmartCardVer5::TSmartCardVer5(TSmartCard &inCard)
...
void TSmartCardVer5::UnlockCard(std::map <int, TSyndCode> SyndCodes)
{
    // Read in entire card block and check it for curruption.
    std::auto_ptr<TMemoryStream> Stream(new TMemoryStream);
    unsigned short CRC = 0;
    unsigned short CalcCRC = 0;
	BlockData.Read(V5_CARD_CRC_DATA_START,CRC);

   if (!StreamCheckCRC(CRC,
                       CalcCRC,
                       BlockData.GetStream(),
                       V5_CARD_VERSION_DATA_START,
                       (V5_CARD_POINTS_DATA_START \
                        + V5_CARD_POINTS_DATA_LENGTH \
                        - V5_CARD_VERSION_DATA_START))) {
...
void TSmartCardVer5::DiagSmartCardBlock(TSmartCardBlockInfo &Info)
{
   // Read in entire card block and check it for curruption.
   std::auto_ptr<TMemoryStream> Stream(new TMemoryStream);
   unsigned short CRC = 0;
...
void TSmartCardVer5::Validate(TSmartCardBlock &inCardBlockData)
{
   ...
   for (int i = V5_CARD_VERSION_DATA_START; i < V5_CARD_VERSION_DATA_START + V5_CARD_VERSION_DATA_LENGTH  ; i++)
   {
   ...
   for (int i = V5_CARD_MEMBER_DATA_START; i < V5_CARD_MEMBER_DATA_START + V5_CARD_MEMBER_DATA_LENGTH  ; i++)
   {
      if(((BYTE *)BlockStream->Memory)[i] != ((BYTE *)CardStream->Memory)[i])
      {
   ...
   // Compare it to the current block Member Data.
   for (int i = V5_CARD_POINTS_DATA_START; i < V5_CARD_POINTS_DATA_START + V5_CARD_POINTS_DATA_LENGTH  ; i++)
   {
      if(((BYTE *)BlockStream->Memory)[i] != ((BYTE *)CardStream->Memory)[i])
      {
   ...
   for (int i = V5_CARD_GUID_START; i < V5_CARD_GUID_START + V5_CARD_GUID_LENGTH  ; i++)
   {
      if(((BYTE *)BlockStream->Memory)[i] != ((BYTE *)CardStream->Memory)[i])
      {
...
void TSmartCardVer5::SetContactInfo(TMMContactInfo inContactInfo)
{
	DirtyContacts = true;
...
TMMContactInfo TSmartCardVer5::GetContactInfo()
{
   ContactInfo.ATRStr = BlockData.CardGuid.GUID();
   return ContactInfo;
}

void TSmartCardVer5::SetVersionInfo()
{
   DirtyVersion = true;
   VersionInfo = SMART_CARD_VERSION_FIVE;
}

void TSmartCardVer5::SetPointsInfo(TContactPoints &inContactPoints)
{
   DirtyPoints = true;
   ContactInfo.Points = inContactPoints;
}

void TSmartCardVer5::SetGUIDInfo(TSmartCardGUID &inGUID)
{
   DirtyGUID = true;
   BlockData.CardGuid.Assign(inGUID);
}

void TSmartCardVer5::Save()
{
   if(!SyndCode.Valid())
   {
   ...
   if(DirtyVersion)
   {
      ...
      BlockData.BlockInfoWrite(V5_CARD_VERSION_DATA_START,V5_CARD_VERSION_DATA_LENGTH,*VersionStream.get());
      UpdateCRC = true;
   }
   ...
   if(DirtyContacts)
   {
      ...
      BlockData.BlockInfoWrite(V5_CARD_MEMBER_DATA_START,V5_CARD_MEMBER_DATA_LENGTH,*ContactStream.get());
      UpdateCRC = true;
   }
   ...
   if(DirtyPoints)
   {
      ...
      BlockData.BlockInfoWrite(V5_CARD_POINTS_DATA_START,V5_CARD_POINTS_DATA_LENGTH,*PointsStream.get());
      UpdateCRC = true;
   }
   ...
   if(DirtyGUID)
   {
      ...
      BlockData.BlockInfoWrite(V5_CARD_GUID_START,V5_CARD_GUID_LENGTH,*GUIDStream.get());
   }
   ...
   { // CRC.
      ...
      BlockData.BlockInfoWrite(V5_CARD_CRC_DATA_START,V5_CARD_CRC_DATA_LENGTH,*CRCStream.get());
   }
   // Begin Actual Card Writes.

   if(DirtyVersion)
   {
      LONG lReturn = CardInfoWrite(V5_CARD_VERSION_DATA_START,V5_CARD_VERSION_DATA_LENGTH,*VersionStream.get());
   ...
   if(DirtyContacts)
   {
      LONG lReturn = CardInfoWrite(V5_CARD_MEMBER_DATA_START,V5_CARD_MEMBER_DATA_LENGTH,*ContactStream.get());
      if (lReturn != SCARD_S_SUCCESS && lReturn != SCARD_M_CHECK_ERROR)
      {
   ...
   if(DirtyPoints)
   {
      LONG lReturn = CardInfoWrite(V5_CARD_POINTS_DATA_START,V5_CARD_POINTS_DATA_LENGTH,*PointsStream.get());
      if (lReturn != SCARD_S_SUCCESS && lReturn != SCARD_M_CHECK_ERROR)
      {
   ...
   if(DirtyGUID)
   {
      LONG lReturn = CardInfoWrite(V5_CARD_GUID_START,V5_CARD_GUID_LENGTH,*GUIDStream.get());
      if (lReturn != SCARD_S_SUCCESS && lReturn != SCARD_M_CHECK_ERROR)
      {
   ...
   if(UpdateCRC)
   { // CRC.
      LONG lReturn = CardInfoWrite(V5_CARD_CRC_DATA_START,V5_CARD_CRC_DATA_LENGTH,*CRCStream.get());
      if (lReturn != SCARD_S_SUCCESS && lReturn != SCARD_M_CHECK_ERROR)
      {
...
void TSmartCardVer5::StreamGetContact(TMemoryStream &Stream)
{
   Stream.Clear();
   BlockData.BlockInfoRead(V5_CARD_MEMBER_DATA_START,V5_CARD_MEMBER_DATA_LENGTH,Stream);
   Stream.Position = 0;
}

void TSmartCardVer5::StreamGetPoints(TMemoryStream &Stream)
{
   Stream.Clear();
   BlockData.BlockInfoRead(V5_CARD_POINTS_DATA_START,V5_CARD_POINTS_DATA_LENGTH,Stream);
   Stream.Position = 0;
}

void TSmartCardVer5::Restore(TSmartCardBlock &RestorePoint)
{
...
/* We rely on the contact information already being loaded when the v5
 * ``interface'' is instantiated.
 */
void TSmartCardVer5::Upgrade(std::map <int, TSyndCode> SyndCodes)
{
