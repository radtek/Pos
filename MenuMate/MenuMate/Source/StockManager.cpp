...
void TManagerStock::UpdateStock(Database::TDBTransaction &DBTransaction,TList *OrderObjects)
{
	try
	{
...
void TManagerStock::UpdateStock(Database::TDBTransaction &DBTransaction,TItemComplete *Order)
{
	try
	{
		...
		for(int i = 0;  i < Order->SalesRecipesToApply->Count ; i++)
		{
			...
			if(Recipe->Enabled)
			{
				...
				IBInternalQuery->ParamByName("ITEM_QTY")->AsFloat					= Recipe->Qty;
...
//------------------------------------------------------------------------------
void TManagerStock::UpdateStock(Database::TDBTransaction &DBTransaction,TItemCompleteSub *Order)
{
	try
	{
		TIBSQL *IBInternalQuery = DBTransaction.Query(DBTransaction.AddQuery());
