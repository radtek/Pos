...
__fastcall TXeroInvoice::TXeroInvoice() : TObject()
{
    ...
    FInvoicePayments     = new TObjectList();
}
//---------------------------------------------------------------------------
...
{
	FInvoiceItems->Clear();
    FInvoicePayments->Clear();

	delete FInvoiceContact;
	delete FInvoiceItems;
    delete  FInvoicePayments;
}
//---------------------------------------------------------------------------
...
void TXeroInvoice::writeInvoiceStatus( AnsiString inInvoiceStatus )
{
...
TXeroInvoiceItem* TXeroInvoice::readInvoicePayment( int inIndex )
{
	return ( TXeroInvoiceItem* )FInvoicePayments->Items[inIndex];
}
...
bool TXeroIntegration::SendInvoice( TXeroInvoice *inXeroInvoice )
{
    ...
    // add payment node
	TiXmlElement *itemsPaymentNode = new TiXmlElement("Payments");
    rootNode->LinkEndChild( itemsPaymentNode );
	AddPayment( itemsPaymentNode, inXeroInvoice );
...
void TXeroIntegration::AddInvoiceAttrs( TiXmlElement *inInvoiceNode, TXeroInvoice *inXeroInvoice )
{
    ...
    SetNodeAttr( inInvoiceNode, "status",         inXeroInvoice->InvoiceStatus.c_str() );
	...
	SetNodeAttr( inInvoiceNode, "date",           inXeroInvoice->InvoiceDate.FormatString( "dd/mm/yyyy hh:mm:ss"));
	SetNodeAttr( inInvoiceNode, "dueDate",        inXeroInvoice->InvoiceDueDate.FormatString( "dd/mm/yyyy hh:mm:ss"));
}
//----------------------------------------------------------------------------
...
{
	AnsiString unitAmountStr = FormatFloat( "0.0000", inInvoiceItem->UnitAmount );
	AnsiString qtyStr        = FormatFloat( "0.0000", inInvoiceItem->Quantity   );
	AnsiString taxStr        = FormatFloat( "0.0000", inInvoiceItem->TaxAmount  );
...
AnsiString TXeroIntegration::InvoiceTypeToStr( TXeroInvoiceType inInvoiceType )
{
	switch( inInvoiceType )
	{
         ...
         case xitACCRECCREDIT: return "ACCRECCREDIT";
		 case xitACCPAYCREDIT: return "ACCPAYCREDIT";
		 default       : return "ACCREC";
	}
}

void TXeroIntegration::AddPayment( TiXmlElement *inItemsNode, TXeroInvoice *inXeroInvoice )
{
	// process line items
	for( int i = 0; i < inXeroInvoice->InvoicePaymentCount; i++ )
	{
		TiXmlElement *itemNode = new TiXmlElement("Payment");
		inItemsNode->LinkEndChild( itemNode );
		AddPaymentAttrs( itemNode, inXeroInvoice->InvoicePayment[i] );
	}
}


void TXeroIntegration::AddPaymentAttrs( TiXmlElement *inItemNode, TXeroInvoiceItem *inInvoicePayment )
{
	AnsiString unitAmountStr = FormatFloat( "0.0000", inInvoicePayment->UnitAmount );
	AnsiString qtyStr        = FormatFloat( "0.0000", inInvoicePayment->Quantity   );
	AnsiString taxStr        = FormatFloat( "0.0000", inInvoicePayment->TaxAmount  );

	SetNodeAttr( inItemNode, "accountCode", inInvoicePayment->AccountCode.c_str() );
	SetNodeAttr( inItemNode, "description", inInvoicePayment->Description.c_str() );
	SetNodeAttr( inItemNode, "unitAmount",  unitAmountStr.c_str() );
	SetNodeAttr( inItemNode, "taxAmount",   taxStr.c_str() );
   //	SetNodeAttr( inItemNode, "qty",         qtyStr.c_str() );
}
...
void TXeroInvoice::AddPayment( AnsiString inAccountCode,
							AnsiString inDescription,
							double     inUnitAmount,
							double     inTaxAmount)
{
	TXeroInvoiceItem *invoicePayment = new TXeroInvoiceItem();

	invoicePayment->AccountCode = inAccountCode;
	invoicePayment->Description = inDescription;
	invoicePayment->UnitAmount  = inUnitAmount;
	invoicePayment->TaxAmount   = inTaxAmount;
...
//------------------------------------------
void TXeroInvoice::AddPayment( TXeroInvoiceItem* inInvoicePayment )
{
	FInvoicePayments->Add( ( TObject *)inInvoicePayment );
}


int TXeroInvoice::readInvoicePaymentCount()
{
	return FInvoicePayments->Count;
}
