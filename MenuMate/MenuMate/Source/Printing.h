...
private:
   ...
   int SeatCount; 												// A list of TKitchenCourses. The Courses Present in the current Request.
    ...
    void AddTransferToPrintout(TPrintFormat *pPrinter,TTransferComplete *Transfer,bool isPartialTranferItemOrGuests, bool IsDestToSourceDocketPrint);     // Add Print Transfer for the Table Transfer to Kitchen
   ...
   bool GetPrintouts(Database::TDBTransaction &DBTransaction,TTransferComplete *Transfer,TReqPrintJob *Request,bool IsPartialTranferItemOrGuests = false ,bool IsReversedPartialDocketPrint = false);
