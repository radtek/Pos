...
{
if((!TDeviceRealTerminal::Instance().ManagerMembership->ManagerSmartCards->CardInserted) && (!TGlobalSettings::Instance().LoyaltyMateEnabled))
{
        ...
        try
          {
              ...
              check_dup_prox_query = DBTransaction.Query(DBTransaction.AddQuery());

              check_dup_prox_query->Close();
              check_dup_prox_query->SQL->Add(
	            "select name from contacts where prox_card = :prox_card and contacts_key <> :contacts_key;");
              check_dup_prox_query->ParamByName("prox_card")->AsString = new_prox_str;
              check_dup_prox_query->ParamByName("contacts_key")->AsInteger = Info.ContactKey;
              check_dup_prox_query->ExecQuery();

              if (!check_dup_prox_query->RecordCount)
              {
		        Info.ProxStr = new_prox_str;
              }
              else
