...
TManagerMembership::TManagerMembership(Database::TDBControl &inDBControl, TModules &inModules,
   TManagerVariable &inVars) : DBControl(inDBControl), Modules(inModules), MembershipSystem(NULL), Vars(inVars)
{
}
...
void TManagerMembership::Initialise(Database::TDBTransaction &DBTransaction)
{
   ...
   case MembershipTypeMenuMate:
	  MembershipSystem.reset(new TMembership(Modules, Vars));
	  break;
   case MembershipTypeERS:
	  MembershipSystem.reset(new TMembershipERS(Modules, Vars, IBMembershipDatabase));
	  break;
   case MembershipTypeEBet:
	  MembershipSystem.reset(new TMembershipEBet(Modules, Vars, TGlobalSettings::Instance().MembershipDatabaseIP, TGlobalSettings::Instance().MembershipDatabasePort, TGlobalSettings::Instance().EBetID));
	  break;
   default:
	  MembershipSystem.reset(new TMembership(Modules, Vars)); ;
   }
...
Currency TManagerMembership::GetValue(Database::TDBTransaction &DBTransaction, int inContactKey)
{
   Currency RetVal = 0;
   try
   {
...
void TManagerMembership::GetReportMemberInfo(TMMContactInfo &Member, TSyndCode &SyndCode, UnicodeString Title, TStringList *Report)
{
   ...
   Report->Add(TempRow);
   TempRow = LoadStr(TABLE_ROW4);
   TempRow = AnsiReplaceStr(TempRow, "%ROWTITLE1%", "Current Points");
   TempRow = AnsiReplaceStr(TempRow, "%ROWCONTENT1%", FormatFloat("0.00", Member.Points.CurrentPoints));
   TempRow = AnsiReplaceStr(TempRow, "%ROWTITLE2%", "Syndicate Coding");
   TempRow = AnsiReplaceStr(TempRow, "%ROWCONTENT2%", SyndCode.Name);
   Report->Add(TempRow);
