...
//-------------------------------------------------------------------------------------------------
void TManagerMallSetup::InsertSettingValuesForEvia(Database::TDBTransaction &dbTransaction, int deviceKey, int mallId)
{
    try
    {
        bool isRecordExist = IsSettingExistInDB(dbTransaction, deviceKey, mallId);
        ...
        if(!isRecordExist)
        {
            const int numberOfFields = 10;
             UnicodeString fieldTypes[numberOfFields] =
             {
                "UnicodeString", "UnicodeString", "int", "bool", "UnicodeString", "UnicodeString", "bool", "bool", "bool"
             };
