...
std::vector<double> TWeightStreamKenkoKK300::RecentWeightsList;

TWeightStreamKenkoKK300::TWeightStreamKenkoKK300(unsigned char *Buffer, int BufferSize) : Stream(new TMemoryStream)
{
    Stream->Write(Buffer, BufferSize);
    Stream->Position = 0;
}

TWeightStreamKenkoKK300* TWeightStreamKenkoKK300::Create(unsigned char *Buffer, int BufferSize)
{
    return new TWeightStreamKenkoKK300(Buffer, BufferSize);
}

void TWeightStreamKenkoKK300::GetWeight(TWeight &Weight)
{
    Stream->Position = 0;
    ...
    int lastEndOfTextPosition = UnformattedWeightStr.LastDelimiter("\x28");
    ...
    double dblWeight = StrToFloatDef(WeightStr.Trim(), -1) * 1000;
    if (dblWeight == -1)
    {
...
void TWeightStreamKenkoKK300::GetStable(bool &Stable)
{
    Stable = false;
