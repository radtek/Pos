...
namespace AccountingIntegration.Xero
{
    ...
    public class XeroIntegrationApplicationRunner
    {
        ...
        void InitFolderVars()
        {
            _cacheFolder = XeroFolder(AccountingIntegration.Properties.Settings.Default.XeroIntegrationCacheFolder);
            _retryFolder = XeroFolder(AccountingIntegration.Properties.Settings.Default.XeroIntegrationRetryFolder);
            _successfulFolder = XeroFolder(AccountingIntegration.Properties.Settings.Default.XeroIntegrationSuccessfulFolder);
            _failedFolder = XeroFolder(AccountingIntegration.Properties.Settings.Default.XeroIntegrationFailedFolder);
        }
        ...
        private void SendOldCachedInvoices(string inPath)
        {
            ...
            foreach (string filename in filenames)
            {
                try
                {
                    if (filename.ToUpper().EndsWith(@".XML"))
                    {
                        ServiceLogger.Log("SendOldCachedInvoices");
        ...
        void processXmlFileTimer_Tick(object obj)
        {
            ...
            try
            {
                ServiceLogger.Log("processXmlFileTimer_Tick");
        ...
        private void CreateCreditNode(string inFilename, XeroInvoice invoice)
        {
            ...
            if (createdCreditNote.ValidationStatus == ValidationStatus.Error)
            {
