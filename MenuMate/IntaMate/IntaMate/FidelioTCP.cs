...
namespace IntaMate
{
    class PMSInterfaceFidelioTCP : IntaMate.PMSInterface
    {
        public bool Active = false;

        public PMSInterfaceFidelioTCP()
        {
        }

        protected override I_IO_Interface CreateTransport()
        {
            return new InterfaceIOTCP();
        }
        ...
        public override bool IsConnected()
        {
            ...
            try
            {
                ...
                {
                    logger.Info("\rMicrosServer Online: Ack Received");
                    RetVal = true;
                }
                else if (Ack == ProtocolUtility.NAK)
                {
                    logger.Info("\rMicrosServer OffLine: Nak Received");
                }
                else
                {
                    logger.Info("\rMicrosServer OffLine: Unexpected Response");
                }
            }
            catch (System.TimeoutException ex)
            {
                logger.Error("\rMicrosServer OffLine: No Reply " + ex.Message);
            }
            catch (System.Exception ex)
            {
                logger.Error("\rMicrosServer OffLine: " + ex.Message);
            }
            return RetVal;
        }
