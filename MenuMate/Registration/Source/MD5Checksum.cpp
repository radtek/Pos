...
RETURNS:		UnicodeString : the hexadecimal MD5 checksum for the specified file
ARGUMENTS:		UnicodeString& strFilePath : the full pathname of the specified file
NOTES:			Provides an interface to the CMD5Checksum class. 'strFilePath' name should 
				...
				exception.
*****************************************************************************************/
AnsiString CMD5Checksum::GetMD5(const AnsiString Data)
{
	CMD5Checksum MD5Checksum;
	MD5Checksum.Update( Data.c_str(), Data.Length() );
...
*****************************************************************************************/
AnsiString CMD5Checksum::GetMD5(TMemoryStream *Stream)
{
   ...
   while ((nLength = Stream->Read( Buffer, nBufferSize )) > 0 )
   {
...
AnsiString CMD5Checksum::Final()
{
	...
	for ( int i=0; i < nMD5Size; i++) 
	{
		...
		if (lpszMD5[i] == 0) {
			...
			Str.printf("0%x",lpszMD5[i]);
		}
		else {
			Str.printf("%x",lpszMD5[i]);
		}
		strMD5 += Str;
	}
	return strMD5;
