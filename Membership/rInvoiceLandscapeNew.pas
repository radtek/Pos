...
type
  TfrmInvoiceLandscapeNew = class(TForm)
    qr: TQuickRep;
    QRBand1: TQRBand;
    ...
    function ExtractSubsExpires(  PrtIndex : Integer ;mGroups,mMembers : ShortString ;
                                  DateFrom,DateTo : TdateTime  ) : Boolean;
    procedure FormDestroy(Sender: TObject);
    procedure bandDetailBeforePrint(Sender: TQRCustomBand;
      var PrintBand: Boolean);
    procedure subDetailAfterPrint(Sender: TQRCustomBand;
      BandPrinted: Boolean);
...
var
  frmInvoiceLandscapeNew: TfrmInvoiceLandscapeNew;
...
Uses
    ...
    Global; //StockMaster
{ **************************************************************************** }
function TfrmInvoiceLandscapeNew.ExtractMember(  PrtIndex,PartPay : Integer ;
                                              InvoiceMembers : ShortString ;
                                              Discount : Extended  ) : Boolean;
             // use Amount as the Member could have paid ahead
const SQL1 = 'SELECT m.MAILING_LIST_CODE,m.MEMBER,m.FULL_NAME,s.DESCRIPTION,' +
...
{ **************************************************************************** }
function TfrmInvoiceLandscapeNew.ExtractSubsRun(  PrtIndex : Integer   ) : Boolean;
...
{ **************************************************************************** }
function TfrmInvoiceLandscapeNew.ExtractSubsExpires(  PrtIndex : Integer; mGroups,mMembers : ShortString;
                                                   DateFrom,DateTo : TdateTime ) : Boolean;
{const SQL2 = 'insert into Lucky_member SELECT DISTINCT(M.MAILING_LIST_CODE),m.MEMBER ' +
             'FROM MEMBERS M, SUBSCRIPTIONS S , M_SUBS ms %s where m.MEMBER = MS.MEMBER ' +
             'and MS.SUBSCRIPTION = s.SUBSCRIPTION %s and S.KEEP_EXPIRES_CURRENT <> ''Y'' ' +
             'and M.EXPIRES BETWEEN  ''%s'' AND ''%s'' and m.DECEASED <> ''Y'' %s';
     }
const  SQL1 = 'delete from LUCKY_MEMBER';    
       SQL2 = 'insert into LUCKY_MEMBER SELECT distinct(M.MAILING_LIST_CODE),' +
             'M.MAILING_LIST_CODE FROM MEMBERS M, SUBSCRIPTIONS s,M_SUBS ms %s ' +
             'where m.MEMBER = MS.MEMBER and MS.SUBSCRIPTION = s.SUBSCRIPTION ' +
             ' %s and S.KEEP_EXPIRES_CURRENT <> ''Y'' and M.EXPIRES BETWEEN  ' +
             '''%s'' AND ''%s'' and m.DECEASED <> ''Y'' %s';
       SQL3 = 'select l.MEMBER from lucky_member l , members m ' +
              'where l.MEMBER = m.MAILING_LIST_CODE and l.MEMBER = m.MEMBER order by m.Surname';

       SQL4 = 'update subscriptions set KEEP_EXPIRES_CURRENT = ''N'' where KEEP_EXPIRES_CURRENT is null';

{       SQL5 = 'SELECT lm.MEMBER,m.MAILING_LIST_CODE,m.FULL_NAME,s.DESCRIPTION,' +
              'ms.AMOUNT,0 as PART_PAY_DUE, ms.AMOUNT_PAID,(s.FEE - ms.AMOUNT_PAID) as AMOUNT_DUE, '+
              '(ms.AMOUNT - ms.AMOUNT_PAID) as AMOUNT_DUE_AMOUNT,m.EXPIRES,s.FEE,s.DISCOUNTABLE ' +
              'from MEMBERS m,M_SUBS ms,LUCKY_MEMBER lm, SUBSCRIPTIONS s where lm.MEMBER = m.MEMBER ' + 
              'and m.MEMBER = ms.MEMBER and MS.SUBSCRIPTION = s.SUBSCRIPTION ' +   
              'and (ms.AMOUNT - ms.AMOUNT_PAID) > 0.00 order by m.MAILING_LIST_CODE,lm.MEMBER';}
              
       SQL5 = 'SELECT distinct m.MEMBER,m.MAILING_LIST_CODE,m.FULL_NAME,s.DESCRIPTION, ' +
              'ms.AMOUNT,0 as PART_PAY_DUE, ms.AMOUNT_PAID,(s.FEE - ms.AMOUNT_PAID) as AMOUNT_DUE, ' + 
              '(ms.AMOUNT - ms.AMOUNT_PAID) as AMOUNT_DUE_AMOUNT,m.EXPIRES,s.FEE,s.DISCOUNTABLE ' +
              'from MEMBERS m,M_SUBS ms,LUCKY_MEMBER lm, SUBSCRIPTIONS s %s where m.MAILING_LIST_CODE = :member ' +
              'and m.MEMBER = ms.MEMBER %s and MS.SUBSCRIPTION = s.SUBSCRIPTION ' +
           //   'and (s.FEE - ms.AMOUNT_PAID) > 0.00 and m.EXPIRES between ''%s'' and ''%s'' ' +
              'and s.ON_INVOICE = ''Y'' and m.EXPIRES between ''%s'' and ''%s'' ' +
              'and m.deceased <> ''Y'' %s';
...
begin
     ...
     FExtractByMember := False;
     // delete from lucky member
     dm.sqlExecQuery(dm.qryGeneral,SQL1);
     // need to set  KEEP_EXPIRES_CURRENT to N - if all fields are null then the 
     // query does not work
     dm.sqlExecQuery(dm.qryGeneral,SQL4);
     // every Parent Code that has a Member who meets the criteria - NOT sorted by Surname
     // cant sort on the INSERT 
     ...
     dm.qryReport1.DataSource := dm.dsrReport;
     // every Parent Code that has a Member who meets the criteria - sorted by Surname
     dm.qryOpen(dm.qryReport,SQL3);
     dm.qryReport1.Close;
     // every Member who meets the criteria
     if mGroups <> '' then begin
        GroupsFieldsStr := format(GroupsFields,[mGroups]);
        dm.qryOpen(dm.qryReport1,format(SQL5,[GroupsTable,GroupsFieldsStr,GetShortDate(DateFrom),GetShortDate(DateTo),mMembers]));
     end else
        dm.qryOpen(dm.qryReport1,format(SQL5,['','',GetShortDate(DateFrom),GetShortDate(DateTo),mMembers]));
     qr.PrinterSettings.PrinterIndex := PrtIndex;
     result    := True;
end;
{ **************************************************************************** }
procedure TfrmInvoiceLandscapeNew.qrStartPage(Sender: TCustomQuickRep);
...
begin
     ...
     if dm.sqlExecQueryCheckEOF(dm.qryGeneral,format(SQL1,[dm.qryReport.FieldByName('MEMBER').AsInteger,dm.qryReport.FieldByName('MEMBER').AsInteger])) then begin
...
{ **************************************************************************** }
procedure TfrmInvoiceLandscapeNew.qrBeforePrint(Sender: TCustomQuickRep; var PrintReport: Boolean);
...
begin
     ...
     if FixedExpirySystem then begin
        ...
        if (Global.MemberExpiry - 30) <= date then
...
{ **************************************************************************** }
procedure TfrmInvoiceLandscapeNew.bandGroupFooterAfterPrint(Sender: TQRCustomBand;   BandPrinted: Boolean);
begin
...
{ **************************************************************************** }
procedure TfrmInvoiceLandscapeNew.bandDetailAfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
...
{ **************************************************************************** }
procedure TfrmInvoiceLandscapeNew.bandGroupFooterBeforePrint( Sender: TQRCustomBand; var PrintBand: Boolean);
...
{ **************************************************************************** }
procedure TfrmInvoiceLandscapeNew.FormDestroy(Sender: TObject);
begin
     dm.qryReport1.DataSource := nil;
end; 
{ **************************************************************************** }
procedure TfrmInvoiceLandscapeNew.bandDetailBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
...
{ **************************************************************************** }
procedure TfrmInvoiceLandscapeNew.subDetailAfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
     with dm.qryReport1 do begin
         FAmountDue   := FAmountDue  + FieldByName('AMOUNT_DUE').AsFloat;
         FAmountPAid  := FAmountPAid + FieldByName('AMOUNT_PAID').AsFloat;  
