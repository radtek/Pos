...
void TNumericEdit::Value2FormattedText()
{
	try
	{
		...
		{
			Text = MM_Math::CurrencyString(FValue, FDecimalPlaces, true);
		}
		else
		{
			AnsiString Format;
			...
			Text = FormatFloat(Format, MM_Math::RoundFloat(FValue, FDecimalPlaces, FPrecision));
...
//---------------------------------------------------------------------------
void __fastcall TNumericEdit::KeyPress(wchar_t &Key)
{
  AnsiString S;
