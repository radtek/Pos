...
void WINAPI KillMySelf2( DWORD exitcode, bool fRemoveDir )
{
	...
	dei.pfnFreeLibrary = (PROCFREELIBRARY)::FreeLibrary ;
  	// Assume that the subdirectory is NOT to be removed.
	dei.pfnRemoveDirectory = NULL;
	dei.pfnDeleteFile = (PROCDELETEFILE)::DeleteFile;
  ::GetModuleFileName( dei.hinstExe, dei.File, MAX_PATH );
  if ( fRemoveDir )
  {	// The subdirectory should be removed.
    dei.pfnRemoveDirectory = (PROCREMOVEDIRECTORY)::RemoveDirectory;
#ifdef UNICODE
	  wsccpy( dei.Dir, dei.File );
    *wscrchr( dei.Dir, '\\' ) = 0;
#else
    ...
    *strrchr( dei.Dir, '\\' ) = 0;
	...
	dei.pfnExitProcess = (PROCEXITPROCESS)::ExitProcess;
#else
