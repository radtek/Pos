  ...
  : TForm(Owner)
{
  // code added for static linking in TurboC++ 2006 Explorer
	...
	ZipBuilder->OnMessage = ZipBuilderMessage;
	ZipBuilder->OnSetNewName = ZipBuilderSetNewName;
...
//---------------------------------------------------------------------------
AnsiString TMainForm::RandomName(void)
{
  AnsiString Result;
  for(int i = 0; i < 3 + random(4); ++i)
  {
    Result += AnsiString(char('A' + random(25)));
  }
  return Result + ".txt";
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::CrFBtnClick(TObject* /*Sender*/)
{
  ...
  try
  {
    for(int i = 0; i < 4; ++i)
    {
      ...
      AnsiString fn = SamplePath + SampleNames[i];
...
void __fastcall TMainForm::ShContBtnClick(TObject* /*Sender*/)
{
  ...
  for(int idx = ZipBuilder->Count-1; idx >= 0; --idx)
  {
    ...
    AnsiString fn = NoPrefix(Entry->FileName);
...
void __fastcall TMainForm::ChangeBtnClick(TObject* /*Sender*/)
{
  ...
  try
  {
    for(int i = 0; i < 1 + random(4); ++i)
    {
      int j = random(4);
      strngs->Clear();
      AnsiString fn = SamplePath + SampleNames[j];
      ...
      AnsiString &OldFileName, bool &IsChanged)
{
  ...
  {
    AnsiString Dirtemp = ExtractFilePath(OldFileName);
    Dirtemp = NoPrefix(Dirtemp);
    AnsiString Timedir = "\\#" + FormatDateTime("mmddhhnn", Now()) + AnsiString("#\\");
    Dirtemp += Timedir;
...
//---------------------------------------------------------------------------
bool TMainForm::FileInZip(AnsiString &File)
{
...
//---------------------------------------------------------------------------
AnsiString TMainForm::NoPrefix(AnsiString& Path)
...
void __fastcall TMainForm::DelBtnClick(TObject* /*Sender*/)
{
  ...
  if(FindFirst(SamplePath + "*.txt", faAnyFile, SRec) == 0)
  {
    do
    {
      DeleteFile(SamplePath + AnsiString(SRec.Name));
