...
// program link changed for TurboC++ 2006 Explorer
...
void __fastcall TMainForm::SetZipTotals( void )
{
	...
	if(us) StringGrid1->Cells[4][ZipBuilder1->Count + 1] =
     IntToStr(static_cast<unsigned int>(100 - cs * 100 / us - ((cs * 100 % us) >= us / 2))) + " % ";
...
void __fastcall TMainForm::FillGrid( void )
{
	...
	for(int i = 1; i <= ZipBuilder1->Count; i++)
  {
		...
		if(us) StringGrid1->Cells[4][i] = IntToStr((unsigned int)(100 - cs * 100 / us - ((cs * 100 % us) >= us / 2))) + "% ";
...
void __fastcall TMainForm::ClearGrid(void)
{
  ...
  if(StringGrid1->RowCount == 2) StringGrid1->Rows[1]->SetText("");
  SetZipTotals();
}
//---------------------------------------------------------------------------
