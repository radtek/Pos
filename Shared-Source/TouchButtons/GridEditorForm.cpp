...
void __fastcall TfrmEditGrid::Button5Click(TObject *Sender)
{
	...
	if (SelectedButtons(CurrentButtons))
	{
		...
		{
			if (Application->MessageBox(UnicodeString("Do you wish to clear the glyph/s?").w_str(),UnicodeString("Clear Glyphs?").w_str(), MB_ICONQUESTION + MB_YESNO) == ID_YES)
			{
				for (TButtonsIterator iButton=CurrentButtons.begin(); iButton!=CurrentButtons.end(); iButton++)
				{
...
{
	Scroll = true;
	TouchGrid->Scroll(Touchgrid::sdUp);
}
//---------------------------------------------------------------------------
...
{
	Scroll = true;
	TouchGrid->Scroll(Touchgrid::sdDown);
}
//---------------------------------------------------------------------------
...
{
	Scroll = true;
	TouchGrid->Scroll(Touchgrid::sdLeft);
}
//---------------------------------------------------------------------------
...
{
	Scroll = true;
	TouchGrid->Scroll(Touchgrid::sdRight);
}
//---------------------------------------------------------------------------
