...
{
	const UnicodeString *KeyText = NULL;
...
//---------------------------------------------------------------------------
void __fastcall TTouchNumpad::SetCustomKeyCaption(UnicodeString Caption)
{
...
void __fastcall TNumpadDisplay::Paint()
{
	...
	DrawTextW(Canvas->Handle, GetText().c_str(), -1, &Rect, DT_CENTER | DT_NOPREFIX);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
UnicodeString __fastcall TNumpadDisplay::GetText()
{
	...
	switch (NumpadDisplayMode)
	{
		...
		case dmPIN:
		{
			...
			if (FOnGetPINText)
			{
				FOnGetPINText(this, PIN(), DisplayText);
			}
			break;
		}
	}
...
//---------------------------------------------------------------------------
UnicodeString TNumpadDisplay::GetCurrencyText()
{
	int Pos = 1;
	...
	{
		UnicodeString returnString = "." + FText.SubString(FText.Length() - FDecimalPlaces + 1, FDecimalPlaces);
		UnicodeString tempString   = FText.SubString(1, FText.Length() - FDecimalPlaces);

		while (tempString.Length() >= 4)
		{
			returnString = "," + tempString.SubString(tempString.Length() - 2, 3) + returnString;
			tempString = tempString.SubString(1, tempString.Length() - 3);
		}
...
void TNumpadDisplay::UpdateCurrency(TNumpadKey Key)
{
	switch (Key)
	{
		...
		case nkClear:
		{
			FText = UnicodeString::StringOfChar('0', FDecimalPlaces + 1);
			break;
		}
		default:
		{
...
//---------------------------------------------------------------------------
UnicodeString TNumpadDisplay::GetNumericText()
{
	// No leading '0's
	if (FText != "0" && FText != "0.")
	{
	...
	UnicodeString DispalyText = FText;
...
//---------------------------------------------------------------------------
UnicodeString TNumpadDisplay::Get12HrTimeText()
{
	...
	switch (FText.Length())
	{
		...
		case 1:	break;
		case 2:	Output = UnicodeString(&FText[2], 1) + " :  ";											break;
		case 3:	Output = UnicodeString(&FText[2], 2) + ":  ";											break;
		case 4:	Output = UnicodeString(&FText[2], 2) + ":" + UnicodeString(&FText[4], 1) + " ";	break;
		case 5:	Output = UnicodeString(&FText[2], 2) + ":" + UnicodeString(&FText[4], 2);			break;
	}
	if (FText[1] == 'A')
	{
		Output += " AM";
...
//---------------------------------------------------------------------------
UnicodeString TNumpadDisplay::Get24HrTimeText()
{
	UnicodeString Output = "  :  ";
	switch (FText.Length())
	{
		case 1:	Output = UnicodeString(&FText[1], 1) + " :  ";											break;
		case 2:	Output = UnicodeString(&FText[1], 2) + ":  ";											break;
		case 3:	Output = UnicodeString(&FText[1], 2) + ":" + UnicodeString(&FText[3], 1) + " ";	break;
		case 4:	Output = UnicodeString(&FText[1], 2) + ":" + UnicodeString(&FText[3], 2);			break;
	}
	return Output;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
UnicodeString TNumpadDisplay::GetPINText()
{
/* PIN
   if (FInitialText)
	{
	...
	{
		return UnicodeString::StringOfChar('*', FText.Length());
	}
}
//---------------------------------------------------------------------------
...
TTime TNumpadDisplay::Time()
{
	...
	{
		UnicodeString timeText = FText;
		Word Hour, Min;
		if (timeText.Length() != 5)
		{
			timeText = timeText + UnicodeString::StringOfChar('0', 5 - timeText.Length());
		}
		Hour = StrToInt(UnicodeString(&timeText[2], 2));
		if (timeText[1] == 'P')
		{
			if (Hour != 12)
			{
		...
		Min = StrToInt(UnicodeString(&timeText[4], 2));
	...
	else if (FNumpadDisplayMode == dm24HrTime)
	{
		...
		{
			timeText = timeText + UnicodeString::StringOfChar('0', 4 - timeText.Length());
		}
		Hour = StrToInt(UnicodeString(&timeText[1], 2));
		Min = StrToInt(UnicodeString(&timeText[3], 2));
		return EncodeTime(Hour, Min, 0, 0);
	}
	else
	{
...
Currency TNumpadDisplay::Numeric()
{
	...
	{
		UnicodeString currencyText = GetCurrencyText();
		int commaIndex = currencyText.Pos(",");
		while (commaIndex != 0)
		{
			currencyText.Delete(commaIndex, 1);
            commaIndex = currencyText.Pos(",");
        }
		return StrToCurr(currencyText);
	}
...
void TNumpadDisplay::SetNumeric(TNumeric Numeric)
{
	...
	if (Part < 0) Part = -Part;
	UnicodeString PartStr = IntToStr(Part);
	if (PartStr.Length() < 4)
	{
		PartStr = UnicodeString::StringOfChar('0', 4 - PartStr.Length()) + PartStr;
	}
	...
	switch (FNumpadDisplayMode)
	{
		case dmCurrency:
		{
			if (PartStr.Length() < FDecimalPlaces)
			{
				PartStr = PartStr + UnicodeString::StringOfChar('0', FDecimalPlaces - PartStr.Length());
			}
