...
typedef const std::auto_ptr<TIBQuery>				TcpIBQuery;
//---------------------------------------------------------------------------
...
struct TDBSettings
{
	...
	TDBSettings(
			...
			UnicodeString					iUserName="SYSDBA",
			UnicodeString					iPassword="masterkey",
...
*/
	...
	UnicodeString					CharacterSet;	// eg ISO8859_1
	Ibservices::TProtocol	Protocol;
	TDBType						DBType;
	int							Port;

	UnicodeString					ConnectionString();	// Will throw and exception if it looks invalid.

//	void			Connection(UnicodeString &Server, UnicodeString &FilePath, UnicodeString &FileName);
};
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
	...
	UnicodeString					ConnectionServer();
	UnicodeString					ConnectionPath();
	UnicodeString					ConnectionFile();
private:
	...
	bool							EnablePrivilege(HANDLE h, UnicodeString AName, bool AState);
};
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
...
public:
   ...
   bool                    StartTransaction();
	...
	int							GetGeneratorVal(UnicodeString GeneratorName, int Increment=1);

	void							RegisterQuery(TcpIBSQL &Query, int DatabaseIndex=0);	// Left for backwards compatibility
   void                    RegisterQuery(TcpIBQuery &Query, int DatabaseIndex=0);
   TIBDatabase *GetDefaultDB();
private:
...
struct TIndexDefinition
{
	...
	bool			Unique;
	UnicodeString	ForeignKey;
	UnicodeString	ForeignTable;
};
struct TConstraintDefinition
{
	UnicodeString	TableName;
	UnicodeString	ConstraintType;
	UnicodeString	FieldName;
	UnicodeString	IndexName;
	UnicodeString	ForeignTable;
};

typedef std::vector<UnicodeString> TTableNames;
typedef std::set<UnicodeString> TTableNamesSet;

typedef std::vector<UnicodeString> TGeneratorNames;

typedef std::vector<UnicodeString> TFieldNames;
typedef std::set<UnicodeString> TFieldNameSet;
typedef std::set<UnicodeString> TIndexNames;
typedef std::set<UnicodeString> TIndexFields;
typedef std::set<UnicodeString> TConstraintNames;

typedef std::vector<UnicodeString> TViewNames;
//---------------------------------------------------------------------------
class TDBUtilities
{
...
//---------------------------------------------------------------------------
	...
	void	GetTableNames(TTableNames &TableNames) const;
	void	GetTableNames(std::set<UnicodeString> &TableNames) const;
	void							GetTableNames(TTableNames &TableNames);
	void 							GetGeneratorNames(TGeneratorNames &GeneratorNames);
   void GetGeneratorNames(std::set<UnicodeString> &names) const;

void GetFieldNames(const UnicodeString table_name,
                      std::set<UnicodeString> &field_names) const;

   void GetStoredProcedureNames(std::set<UnicodeString> &names) const;

   void GetViewNames(std::set<UnicodeString> &names) const;
	void							GetFieldNames(UnicodeString TableName, TFieldNames &FieldNames);
	void							GetFieldDefinition(UnicodeString TableName, UnicodeString FieldName, TFieldDefinition &FieldDefinition);
	void							GetIndexNames(UnicodeString TableName, TIndexNames &IndexNames);
	void							GetIndexDefinition(UnicodeString IndexName, TIndexDefinition &IndexDefinition);
	void							GetIndexFields(UnicodeString IndexName, TIndexFields &IndexFields);
	void							GetConstraintNames(UnicodeString TableName, TConstraintNames &ConstraintNames);
	void							GetConstraintDefinition(UnicodeString IndexName, TConstraintDefinition &ConstraintDefinition);
	void							GetViewNames(TViewNames &ViewNames);
...
//---------------------------------------------------------------------------
	...
	bool							StartRepair();
	bool							StartRepair(UnicodeString ServerName, UnicodeString FileName); // Make sure you're not connected to this!
	// Pass it the server name, source file (including path and name), dest file (including path and name)
	bool							StartBackup(UnicodeString ServerName, UnicodeString SourceFile, UnicodeString DestFile, bool Verbose = true,
	...
	bool							StartBackup(UnicodeString DestFilePath, UnicodeString DestFileName, bool Verbose = true,
											TBackupOptions const &BackupOptions =
											TBackupOptions() << NoGarbageCollection << IgnoreLimbo);
	...
	bool							StartBackup(UnicodeString DestFileName, bool Verbose = true,
											TBackupOptions const &BackupOptions =
											TBackupOptions() << NoGarbageCollection << IgnoreLimbo);

	bool							StartRestore(UnicodeString ServerName, UnicodeString SourceFile, UnicodeString DestFile, bool Verbose = true,
											bool ReplaceDB=false);

	bool							StartRestore(UnicodeString ServerName, UnicodeString SourceFile, UnicodeString DestFile, bool Verbose = true,
											TRestoreOptions const &RestoreOptions = TRestoreOptions() << CreateNewDB);

	typedef void __fastcall (__closure *TServiceCompleteEvent)(bool Success, UnicodeString ErrorMsg, TIBServiceType ServiceType);
	typedef void __fastcall (__closure *TServiceGetStatus)(UnicodeString Msg, TIBServiceType ServiceType);
	// Set these for use with threaded service functions.
	TServiceCompleteEvent	OnServiceComplete;
									// void __fastcall TForm1::ServiceComplete(bool Success, UnicodeString ErrorMsg, Database::TIBServiceType ServiceType)
	TServiceGetStatus			OnServiceStatus;
									// void __fastcall TForm1::ServiceStatus(UnicodeString Msg, Database::TIBServiceType ServiceType)
	void							CancelService();
	bool       					ServiceBusy();
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
	...
	mutable TDBTransaction				fDBTransaction;
...
//		inline UnicodeString				ErrorMsg()			{ return fErrorMsg; }
...
//		bool								fComplete;
		UnicodeString						fErrorMsg;
		UnicodeString						fStatusText;
	};
//---------------------------------------------------------------------------
