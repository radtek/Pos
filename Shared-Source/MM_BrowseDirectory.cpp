...
//			SHGetFileInfo(PChar(lParam), 0, &ShellFileInfo, sizeof(ShellFileInfo), SHGFI_PIDL | SHGFI_DISPLAYNAME | SHGFI_TYPENAME);
			...
			SHGetPathFromIDList(reinterpret_cast<LPITEMIDLIST>(lParam), Path);
			FolderDialog->EnableOK(HWindow, DirectoryExists(Path));
			SendMessage(HWindow, BFFM_SETSTATUSTEXT, 0, reinterpret_cast<LPARAM>(Path)); // Not much point. It doesn't work with BIF_NEWDIALOGSTYLE
		}
	}
	return 0;
}
