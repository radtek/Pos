	...
	try
	{
		...
		Query->SQL->Text =
			"Select "
				...
				"Cast(Sum( cast(StockTrans.Unit_Cost as Double Precision) * cast(abs(StockTrans.Qty) as Double Precision ) ) as Numeric(15, 4)) Total_Cost,"
				"cast(Sum( cast(StockTrans.Unit_Cost as Double Precision) * cast(abs(StockTrans.Qty) as Double Precision) * cast(StockTrans.GST_Percent as Double Precision) / 100 ) as numeric(15,4) ) Total_GST "
			"From "
