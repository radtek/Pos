...
//		InflateRect(&CentredRect, -4, -4);
		...
		int TextHeight = DrawTextEx(Bitmap->Canvas->Handle,
		...
		if ((Button->Enabled && FFontShadow) || (!Button->Enabled && FDisabledFontShadow))
		{
			...
			DrawTextEx(	Bitmap->Canvas->Handle,
							Button->Caption.c_str(),
							Button->Caption.Length(),
							&BottomRect,
...
/*		if (Button->Down)
		...
		int TextHeight = DrawTextEx(Buffer->Canvas->Handle,
											 Button->Caption.c_str(),
											 Button->Caption.Length(),
		...
		DrawTextEx(Buffer->Canvas->Handle, Button->Caption.c_str(), Button->Caption.Length(), &CentredRect, DT_WORDBREAK|DT_CENTER|DT_NOPREFIX, NULL);
	}
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TGridButton::SetCaption(AnsiString Caption)
{
	FCaption = Caption;
	Invalidate();
}
