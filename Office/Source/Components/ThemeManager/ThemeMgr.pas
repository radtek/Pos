...
//   Bert Moorthaemer, Rob Schoenaker, John W. Long, Vassiliev V.V., Steve Moss, Torsten Detsch
//----------------------------------------------------------------------------------------------------------------------
...
type
  ...
  // is themed. It uses an own instance of a TThemeServices class to do themed painting. This instance is
  // also accessible by the application so there is no need to create an extra TThemeServices instance (unless you want
  // per window theming).
  ...
  protected
    ...
    procedure RemoveRecreationCandiate(Control: TControl);
...
begin
  ...
  MainManager.RemoveRecreationCandiate(Control);
end;
        ...
        CN_CTLCOLORSTATIC:
          if TAnimate(Control).Transparent then
            ...
            begin
              ...
              {FThemeServices.DrawParentBackground(TWinControl(Control).Handle, ChildDC, nil, False);
    ...
    begin
      case Message.Msg of
        ...
        CM_MOUSELEAVE:
          ...
          if Control is TBitBtn then
            Control.Invalidate;
        CN_DRAWITEM: // Painting for bit buttons.
...
begin
  ...
  for I := 0 to FListeners.Count - 1 do
    //if CompareMem(@AControlMessage, @TControlMessageEvent(FListeners[I]^), SizeOf(TMethod)) then Exit;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // We need a destroy notification for this control since it is not a window control and does therefore
          // not get WM_DESTROY message.
          if Inserting then
            ...
            begin
              ...
              {$ifdef COMPILER_5_UP}
                ...
                begin
                  ...
                  begin
                    List := FFrameList;
                    // We need a notification for this frame about its destruction.
                    if Inserting then
                    begin
                      Control.FreeNotification(Self);
                      CollectControls(Control as TWinControl);
                    end;
                  end;
                end
                else
                    ...
                    {$endif CheckListSupport}
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            List := FFormList;
                            // We need a notification for this form about its destruction.
                            if Inserting then
                            begin
                              Control.FreeNotification(Self);
                              CollectControls(Control as TWinControl);
...
procedure TThemeManager.RemoveRecreationCandiate(Control: TControl);
