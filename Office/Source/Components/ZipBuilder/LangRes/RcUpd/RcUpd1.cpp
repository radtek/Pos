...
//         5.4  2005-06-20 If text of RcMsgXX and RcMsgUS are iqual will show in red color which
//         (BCB5 & D4)     means its not translated. Any entry unchanged will show always in red.
//
   ...
   for(n = 0; n < EditRC->Lines->Count; ++n) {
      ...
      if(i == 0) {
         ...
         if(i > 0)
            ...
            if(XX_File == US_File) continue;
            i = FindRcCode(US_RC, GetRcCode(EditRC->Lines->Strings[n]));
            if(i < 0) continue;
            if(EditRC->Lines->Strings[n] != US_RC->Strings[i]) continue;
            i = 0;
         }
      }
      if(i > 0) EditRC->Lines->Strings[n] = EditRC->Lines->Strings[n].Delete(i, 14);
      EditUpdateColor(EditRC, n, tc);
   }
   EditRC->SelStart = 0;
   EditRC->SelLength = 0;
   EditRC->SetFocus();
...
//---------------------------------------------------------------------------
// Change text color inf EditRc
void TForm1::EditUpdateColor(TRichEdit *EditRC, int n, TColor tc)
{
   int x = EditRC->SelStart;
      ...
      TShiftState Shift)
{
   ...
   if(n < 0)
      ...
      if(XX_File != US_File)
         if(EditRC->Lines->Strings[tp.y] == US_RC->Strings[n])
            EditUpdateColor(EditRC, tp.y, clRed);
   }
}
//---------------------------------------------------------------------------
