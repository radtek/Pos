...
uses
  ...
  TntClasses, TntStdCtrls, TntControls, TntComCtrls;
...
{TNT-WARN TDBRichEdit}
  ...
  protected
    ...
    function SaveAsRTF: Boolean; dynamic;
  protected
...
uses
  SysUtils, Graphics, Forms, {$IFDEF COMPILER_6_UP} Variants, {$ENDIF} TntDB,
  TntActnList, TntGraphics, TntSysUtils, RichEdit, Mask;

type
  THackFieldDataLink_D5_D6_D7 = class(TDataLink)
  protected
    FField: TField;
...
{$IFDEF COMPILER_5}
  THackFieldDataLink = THackFieldDataLink_D5_D6_D7;
{$ENDIF}
{$IFDEF COMPILER_6}
  THackFieldDataLink = THackFieldDataLink_D5_D6_D7;
{$ENDIF}
...
type
  THackDBEdit_D5_D6_D7 = class(TCustomMaskEdit)
  protected
    FDataLink: TFieldDataLink;
...
{$IFDEF COMPILER_5}
  THackDBEdit = THackDBEdit_D5_D6_D7;
{$ENDIF}
{$IFDEF COMPILER_6}
  THackDBEdit = THackDBEdit_D5_D6_D7;
{$ENDIF}
...
function TTntDBEdit.GetPasswordChar: WideString;
begin
  Result := TntCustomEdit_GetPasswordChar(Self, FPasswordChar)
end;

procedure TTntDBEdit.SetPasswordChar(const Value: WideString);
begin
    ...
    try
      ...
      begin
        ...
        if PasswordChar <> '' then
          for I := 1 to Length(S) do S[I] := PasswordChar[1];
...
function TTntDBRichEdit.SaveAsRTF: Boolean;
begin
  Result := False;
...
begin
  if not FMemoLoaded and Assigned(Field) and SaveAsRTF then
  begin
    try
      InternalLoadMemo;
      FMemoLoaded := True;
...
begin
  ...
  begin
    FFocused := Value;
    if not Assigned(Field) or not SaveAsRTF then
      FDataLink.Reset;
  end;
end;
