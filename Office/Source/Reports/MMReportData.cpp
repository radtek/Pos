...
_dayArcBillSubQuery =  	" Select "
                                    ...
                                    "From "
                                        ...
                                        "   where a.Time_Stamp >= :StartTime and a.Time_Stamp < :EndTime group by a.ARCBILL_KEY,a.TABLE_NAME  )ab on ab.ARCBILL_KEY=DayArcBill.ARCBILL_KEY "
                                     "Where "
...
void TdmMMReportData::SetupHalfHourlyDaily(TDateTime StartTime, TDateTime EndTime,TStrings *Terminals)
{
	...
	qrHalfHoulrySummary->SQL->Text =
		...
		"Where "
        " ARCHIVE.ARCHIVE_KEY not in (Select     archive.ARCHIVE_KEY from   (select *from ARCHIVE where ARCHIVE.PRICE=0 and Archive.TIME_STAMP_BILLED >= :StartTime and Archive.TIME_STAMP_BILLED < :EndTime) ARCHIVE left join SECURITY on  SECURITY.SECURITY_REF=ARCHIVE.SECURITY_REF where security.SECURITY_EVENT='CancelY' and Archive.TIME_STAMP_BILLED >= :StartTime and Archive.TIME_STAMP_BILLED < :EndTime ) and     "
...
void TdmMMReportData::SetupDayConsumption(TDateTime StartTime, TDateTime EndTime)
{
	...
	qrDayConsumption->SQL->Text =
        ...
        " Where ARCHIVE.ARCHIVE_KEY not in (Select     archive.ARCHIVE_KEY from (select *from ARCHIVE where ARCHIVE.PRICE=0 and ARCHIVE.TIME_STAMP >= :StartTime and ARCHIVE.TIME_STAMP < :EndTime ) ARCHIVE left join SECURITY on  SECURITY.SECURITY_REF=ARCHIVE.SECURITY_REF where  security.SECURITY_EVENT='CancelY' and ARCHIVE.TIME_STAMP >= :StartTime and ARCHIVE.TIME_STAMP < :EndTime ) and  "
        " (( "
           " COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Non-Chargeable' and "
           "  COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Complimentary') ) and  "
        ...
        " Where DayArchive.ARCHIVE_KEY not in (Select     DayArchive.ARCHIVE_KEY from   (select *from DAYARCHIVE where DAYARCHIVE.PRICE=0 and DAYARCHIVE.TIME_STAMP >= :StartTime and DAYARCHIVE.TIME_STAMP < :EndTime ) DAYARCHIVE left join SECURITY on  SECURITY.SECURITY_REF=DayArchive.SECURITY_REF where  security.SECURITY_EVENT='CancelY' and DAYARCHIVE.TIME_STAMP >= :StartTime and DAYARCHIVE.TIME_STAMP < :EndTime) and  "
        " (( "
          "  COALESCE(DAYARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Non-Chargeable' and   "
           " COALESCE(DAYARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Complimentary') ) and  "
...
void TdmMMReportData::SetupCategoryConsumptionExcSurcharge(TDateTime StartTime, TDateTime EndTime, TStrings *Categories)
{
	...
	"Select "
       ...
       " LEFT JOIN  (SELECT  a.ARCHIVE_KEY,sum(a.DISCOUNTED_VALUE) DISCOUNTED_VALUE,  a.DISCOUNT_GROUPNAME "
        ...
        " Where ARCHIVE.ARCHIVE_KEY not in (Select     archive.ARCHIVE_KEY from   (select *from ARCHIVE where ARCHIVE.PRICE=0  and ARCHIVE.TIME_STAMP >= :StartTime and ARCHIVE.TIME_STAMP < :EndTime) ARCHIVE left join SECURITY on  SECURITY.SECURITY_REF=ARCHIVE.SECURITY_REF where  security.SECURITY_EVENT='CancelY' ) and  "
        "(( "
          "  COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Non-Chargeable' and   "
	...
	qrConsumption->SQL->Text		=	qrConsumption->SQL->Text +
	   ...
	   "	from (  "
        ...
        " Where DayArchive.ARCHIVE_KEY not in (Select     DayArchive.ARCHIVE_KEY from (select *from DAYARCHIVE where DAYARCHIVE.PRICE=0 and DAYARCHIVE.TIME_STAMP >= :StartTime and DAYARCHIVE.TIME_STAMP < :EndTime ) DAYARCHIVE left join SECURITY on  SECURITY.SECURITY_REF=DayArchive.SECURITY_REF where  security.SECURITY_EVENT='CancelY') and  "
        "(( "
          "  COALESCE(DAYARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Non-Chargeable' and   "
...
void TdmMMReportData::SetupUserSales(TDateTime StartTime, TDateTime EndTime, TStrings *Names, TStrings *Groups)
{
	...
	qrUserSales->SQL->Text = qrUserSales->SQL->Text +
		//"Where ARCHIVE.ARCHIVE_KEY not in (Select     archive.ARCHIVE_KEY from   (select *from ARCHIVE where ARCHIVE.PRICE=0) ARCHIVE left join SECURITY on  SECURITY.SECURITY_REF=ARCHIVE.SECURITY_REF where security.SECURITY_EVENT='CancelY' ) and "
        "Where ARCHIVE.ARCHIVE_KEY not in (Select     archive.ARCHIVE_KEY from   (select *from ARCHIVE where ARCHIVE.PRICE=0 and Archive.TIME_STAMP_BILLED >= :StartTime and Archive.TIME_STAMP_BILLED < :EndTime ) ARCHIVE left join SECURITY on  SECURITY.SECURITY_REF=ARCHIVE.SECURITY_REF where security.SECURITY_EVENT='CancelY') and  "
		    " ((  "
            " COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Non-Chargeable' and "
            " COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Complimentary') ) and "
		 //" ARCHIVE.PRICE<>0 and "
...
void TdmMMReportData::SetupUserSalesSummary(TDateTime StartTime, TDateTime EndTime, TStrings *Names)
{
	...
	qrUserSales->SQL->Text =
   	"Select "
        ...
        "cast(Sum(Archive.Cost * Archive.Qty) as Numeric(17,4)) Cost,"
		"cast(Sum((abs(Archive.Qty) * Archive.BASE_PRICE + Archive.DISCOUNT_WITHOUT_TAX) )  - Sum(Archive.Cost) as Numeric(17,4)) Profit, "
        "Cast(Sum(abs(Archive.QTY) * Archive.BASE_PRICE  + COALESCE(Archive.DISCOUNT_WITHOUT_TAX,0)+ COALESCE( (AOT.VAT),0)+COALESCE( (AOT.ServiceCharge),0) + COALESCE( (AOT.OtherServiceCharge),0)) as Numeric(17,4)) SalesIncl "
     "From "
         "Archive  Left Join Security On "
            "Security.Security_Ref = Archive.Security_Ref "
      ...
      "Group By "
   	...
   	"Select "
        ...
        "cast(Sum((DayArchive.Price + DayArchive.Discount) * DayArchive.Qty) as Numeric(17,4)) Price,"
		 ...
		 "cast(Sum((abs(DayArchive.Qty) * DAYARCHIVE.BASE_PRICE  + DayArchive.DISCOUNT_WITHOUT_TAX) ) - Sum(DayArchive.Cost) as Numeric(17,4)) Profit, "
          "Cast(Sum(abs(DayArchive.QTY) * DayArchive.BASE_PRICE  + COALESCE(DayArchive.DISCOUNT_WITHOUT_TAX,0)+ COALESCE( (AOT.VAT),0)+COALESCE( (AOT.ServiceCharge),0) + COALESCE( (AOT.OtherServiceCharge),0)) as Numeric(17,4)) SalesIncl "
	...
	qrUserSales->SQL->Text		=	qrUserSales->SQL->Text +
      "Group By "
   	...
   	"Select "
         ...
         "cast(Sum(Orders.Qty * Orders.Cost) as Numeric(17,4)) Cost,"
		 "cast(Sum((Orders.Qty * Orders.BASE_PRICE+ Orders.Discount) ) - Sum(Orders.Cost) as Numeric(17,4)) Profit, "
          +  _selectSalesIncl + //For Selecting salesIncl column
      "From "
         "Orders  Left Join Security On "
            "Security.Security_Ref = Orders.Security_Ref "
...
void TdmMMReportData::SetupChronological(TDateTime StartTime, TDateTime EndTime, TStrings *Tables, TStrings *Tabs, TStrings *Terminals)
{
	...
	qrChronological->SQL->Text =
		"Select "
			...
			"Archive.Qty, "
			"Archive.Size_Name, "
             "CAST(CAST(CAST(abs(Archive.Qty)AS NUMERIC(17,2)) * CAST(ARCHIVE.BASE_PRICE AS NUMERIC(17,2)) as numeric(17, 4)) + CAST(coalesce(Archive.DISCOUNT_WITHOUT_TAX,0) AS NUMERIC(17,4))+ CAST(coalesce(AOT.VAT,0) AS NUMERIC(17,4)) "
            " + CAST(coalesce(AOT.ServiceCharge,0)AS NUMERIC(17,4)) + CAST(coalesce(AOT.OtherServiceCharge,0)AS NUMERIC(17,4)) as numeric(17, 4)) as Price, "
...
"LEFT JOIN ( "						   //Join With Tax
	...
	qrChronological->SQL->Text =	qrChronological->SQL->Text +
		...
		"Select "
			...
			"DayArchive.Qty,"
			"DayArchive.Size_Name,"
			"CAST(CAST(CAST(abs(DayArchive.Qty)AS NUMERIC(17,2)) * CAST(DAYARCHIVE.BASE_PRICE AS NUMERIC(17,2)) as numeric(17, 4)) + CAST(coalesce(DayArchive.DISCOUNT_WITHOUT_TAX,0) AS NUMERIC(17,4))+ CAST(coalesce(AOT.VAT,0) AS NUMERIC(17,4)) "
            " + CAST(coalesce(AOT.ServiceCharge,0)AS NUMERIC(17,4)) + CAST(coalesce(AOT.OtherServiceCharge,0)AS NUMERIC(17,4)) as numeric(17, 4)) as Price, " 	
	...
	qrBillPayments->SQL->Text =
		...
		"From "
            ...
            "left join (Select "
                    ...
                    "From POINTSTRANSACTIONS "
                    "left join contacts c on c.contacts_key = POINTSTRANSACTIONS.contacts_key "
                    " where POINTSTRANSACTIONS.Time_Stamp >= :StartTime And   "
			"POINTSTRANSACTIONS.Time_Stamp < :EndTime   "
                    "Group By 1,2 "
                    "union all "
                    "Select ab.INVOICE_NUMBER, "
                    "case when (c.name is null) then 'Non-member transaction'else (c.name ||' '|| c.LAST_NAME) end billed_to "
...
void TdmMMReportData::SetupDiscounts(TDateTime StartTime, TDateTime EndTime,TStrings *Discounts,TStrings *Locations)
{
	...
	qrDiscounts->SQL->Text =
	 	"Select "
            ...
            "Cast(round(sum((abs(Archive.QTY ) * Archive.BASE_PRICE +COALESCE(AOT.VAT,0)+COALESCE( AOT.ServiceCharge,0) +  "
...
"LEFT JOIN ( "
      ...
      " group by "
		...
		"Select "
			...
			"DAYARCBILL.Invoice_Number, "
             ...
             "Cast(round(sum((abs(DAYARCHIVE.QTY ) * DAYARCHIVE.BASE_PRICE +COALESCE(AOT.VAT,0)+COALESCE( AOT.ServiceCharge,0) + COALESCE( AOT.OtherServiceCharge,0)+ COALESCE(DAYARCHIVE.DISCOUNT_WITHOUT_TAX,0) "
             ")),2) as Numeric(17,4)) Total,"
	...
	qrDiscountGrandTotal->SQL->Text =
        ...
        "cast(round(sum(Archive.DISCOUNT),2) as numeric(17,4))DiscountAmount, "
   ...
   " From "
        ...
        "Left Join Archive On   "
              ...
              "FROM SECURITY a where a.SECURITY_EVENT='Discounted By'and a.TIME_STAMP >= :StartTime and a.TIME_STAMP  < :EndTime )SECURITY  on SECURITY.SECURITY_REF=Archive.SECURITY_REF  "
        "LEFT JOIN (  "
				   "	 SELECT    "
					 "	ARCORDERTAXES.ARCHIVE_KEY,  "
					 "	MIN(CASE WHEN ARCORDERTAXES.TAX_TYPE = 0 THEN ARCORDERTAXES.TAX_VALUE END) AS VAT, "
    ...
    qrDiscountGrandTotal->SQL->Text        = qrDiscountGrandTotal->SQL->Text +
		"Union All "
             ...
             "Cast(round(sum((abs(DAYARCHIVE.QTY ) * DAYARCHIVE.BASE_PRICE +COALESCE(AOT.VAT,0)+COALESCE( AOT.ServiceCharge,0) + COALESCE( AOT.OtherServiceCharge,0)+ "
		  ...
		  "cast(round(sum(DayArchive.DISCOUNT),2) as numeric(17,4))DiscountAmount, "
...
for (int i=0; i<Locations->Count; i++)
	...
	qrDiscountGrandTotal->SQL->Text =
         ...
         "Select   "
          "Cast((round(sum(ARCHIVE.QTY * ARCHIVE.BASE_PRICE +COALESCE(AOT.VAT,0)+COALESCE( AOT.ServiceCharge,0) + COALESCE( AOT.OtherServiceCharge,0)+ "
            " COALESCE(ARCHIVE.DISCOUNT_WITHOUT_TAX,0)),2)) as Numeric(17,4)) Total,  "
   ...
   " From "
    ...
    qrDiscountGrandTotal->SQL->Text        = qrDiscountGrandTotal->SQL->Text +
		"Union All "
          		...
          		"Select "
            " Cast((round(sum(DAYARCHIVE.QTY * DAYARCHIVE.BASE_PRICE +COALESCE(AOT.VAT,0)+COALESCE( AOT.ServiceCharge,0) + COALESCE( AOT.OtherServiceCharge,0)+ "
            " COALESCE(DAYARCHIVE.DISCOUNT_WITHOUT_TAX,0)),2)) as Numeric(17,4)) Total, "
...
void TdmMMReportData::SetupLoyaltyHistoryVIPCredit(TDateTime StartTime, TDateTime EndTime,TStrings *Locations)
{
	...
	qrLoyaltyHistory->SQL->Text =
		"Select "
			...
			"Archive.Time_Stamp_Billed Time_Stamp, "
			"cast(sum(PT.Total) as numeric(17, 4)) Total_Points,"
...
void TdmMMReportData::SetupSalesSummaryC(TDateTime StartTime, TDateTime EndTime, TStrings *Locations)
{
    ...
    "ABS(CASE WHEN ARCORDERDISCOUNTS.NAME = 'Directors' OR ARCORDERDISCOUNTS.NAME = 'Food Wastage' OR ARCORDERDISCOUNTS.NAME = 'Bar Wastage' OR ARCORDERDISCOUNTS.NAME = 'Food Panda' THEN SUM(ARCORDERDISCOUNTS.DISCOUNTED_VALUE) ELSE Sum(Cast(COALESCE(ARCHIVE.COST,0) * COALESCE(ARCHIVE.QTY ,0) As Numeric(17, 4))) END) TOTALDISCOUNT, "
...
void TdmMMReportData::SetupSalesSummaryByLocation(TDateTime StartTime, TDateTime EndTime, TStrings *Locations)
{
	...
	qrSalesSummary->SQL->Text = qrSalesSummary->SQL->Text +
           ...
           //" Cast(Sum(Archive.QTY * Archive.BASE_PRICE+Archive.Qty ) as Numeric(17,4)) PriceExc ,  "
             "   Cast(Sum(abs(Archive.QTY) * Archive.BASE_PRICE  + COALESCE(Archive.DISCOUNT_WITHOUT_TAX,0)) as Numeric(17,4)) Sales_Exc , "
...
//-----
    ...
    qrCancelsAdj->SQL->Text =
            ...
            "Select "
            "OrdSec.Terminal_Name,  "
            "cast(sum(Archive.PRICE_LEVEL0 * Archive.Qty) as numeric(17, 4)) Price, "
            "cast(sum(Archive.Qty) as numeric(17, 4)) keyvalue "
      "From "
       "Archive "
       "Left Join ArcBill On Archive.ArcBill_Key = ArcBill.ArcBill_Key "
       "Left Join Security OrdSec On OrdSec.Security_Ref = Archive.Security_Ref "
      "Where "
          "Archive.TIME_STAMP_BILLED >= :StartTime and  "
       "Archive.TIME_STAMP_BILLED < :EndTime and  "
       "(OrdSec.Security_Event = 'Cancel' or OrdSec.Security_Event = 'CancelY' ) ";
        ...
        qrCancelsAdj->SQL->Text	=	qrCancelsAdj->SQL->Text + "GROUP BY 1 ";
...
//-------for refunds
   ...
   qrRefunds->SQL->Text =
	...
	qrRefunds->SQL->Text = qrRefunds->SQL->Text +
        ...
        "Cast( sum(a.PRICE*a.QTY) as Numeric(17,4)) Total_Price, "
        "Cast(sum(a.QTY) as Numeric(17,4)) keyvalue  "
    "From  "
    "Archive a Inner Join  Security On "
        "a.Security_Ref = Security.Security_Ref  "
    "Where "
			"a.TIME_STAMP_BILLED >= :StartTime and  "
		 	"a.TIME_STAMP_BILLED < :EndTime and "
			"(Security.Security_Event = 'Credit' or Security.Security_Event = 'WriteOff' ) ";

    if (Locations && Locations->Count > 0)
    {
        qrRefunds->SQL->Text	=	qrRefunds->SQL->Text + "and (" +
                                    ParamString(Locations->Count, "a.ORDER_LOCATION ", "LocationParam") + ")";
    }
     qrRefunds->SQL->Text	=	qrRefunds->SQL->Text + " GROUP BY 1";
 ...
 void TdmMMReportData::SetupDSRSummary(TDateTime StartTime, TDateTime EndTime,TStrings *Menus)
{
       ...
       "and ARCBILL.TIME_STAMP >= :StartTime and  ARCBILL.TIME_STAMP < :EndTime  "
		...
		"from (Select ARCORDERDISCOUNTS.NAME , MIN(CASE WHEN ARCORDERDISCOUNTS.DISCOUNTED_VALUE <0 AND ARCHIVE.DISCOUNT != 0 THEN  "
		"coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/ARCHIVE.DISCOUNT),0) ELSE coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/1),0) END)  AS Discount_Without_Tax,MIN(CASE WHEN ARCORDERDISCOUNTS.DISCOUNTED_VALUE <0  AND ARCHIVE.DISCOUNT != 0 THEN  "
		"coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE-(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/ARCHIVE.DISCOUNT)),0) ELSE coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE-(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/1)),0) END)  AS Tax_On_Discount "
		"From ARCORDERDISCOUNTS  left join Archive on archive.ARCHIVE_KEY=ARCORDERDISCOUNTS.ARCHIVE_KEY  "
		"where   "
		"COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Non-Chargeable' and  "
		...
		"from ( Select ARCORDERDISCOUNTS.NAME , MIN(CASE WHEN ARCORDERDISCOUNTS.DISCOUNTED_VALUE >0 AND ARCHIVE.DISCOUNT != 0   THEN  "
		"coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/ARCHIVE.DISCOUNT),0) ELSE coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/1),0)  END)  AS Surcharge_Without_Tax,MIN(CASE WHEN ARCORDERDISCOUNTS.DISCOUNTED_VALUE >0 AND ARCHIVE.DISCOUNT != 0  THEN  "
		"coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE-(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/ARCHIVE.DISCOUNT)),0) ELSE coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE-(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/1)),0) END)  AS Tax_On_Surcharge "
		"From ARCORDERDISCOUNTS  left join Archive on archive.ARCHIVE_KEY=ARCORDERDISCOUNTS.ARCHIVE_KEY  "
		"where   "
		"COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Non-Chargeable' and  "
		"COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Complimentary'  and  ARCORDERDISCOUNTS.DISCOUNTED_VALUE >0 and "
...
{    	qrDSRSum->Close();
	...
	"from (Select ARCORDERDISCOUNTS.NAME,MIN(CASE WHEN ARCORDERDISCOUNTS.DISCOUNTED_VALUE <0 AND ARCHIVE.DISCOUNT != 0 THEN "
	"coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/ARCHIVE.DISCOUNT),0)  ELSE coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/1),0) END  )  AS Discount_Without_Tax "
	...
	"from ( Select  ARCORDERDISCOUNTS.NAME, MIN(CASE WHEN ARCORDERDISCOUNTS.DISCOUNTED_VALUE > 0  AND ARCHIVE.DISCOUNT != 0 THEN  "
	"coalesce((ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/ARCHIVE.DISCOUNT)),0) ELSE coalesce(ARCORDERDISCOUNTS.DISCOUNTED_VALUE*(ARCHIVE.DISCOUNT_WITHOUT_TAX/1),0) END )  AS Surcharge_Without_Tax "
	"From ARCORDERDISCOUNTS  left join Archive on archive.ARCHIVE_KEY=ARCORDERDISCOUNTS.ARCHIVE_KEY  "
	"where   "
	"COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Non-Chargeable' and  "
	"COALESCE(ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME,0)<> 'Complimentary'  and  ARCORDERDISCOUNTS.DISCOUNTED_VALUE >0  and "
	 ...
	 qrSalesSummaryD->SQL->Text =
        ...
        "Select "
           ...
           "ZEDS.TIME_STAMP,  "
            ...
            "SUM(CASE When ARCHIVENew.price=0  Then COALESCE(CANCEL_AMOUNT.CANCEL_TOTAL,0) Else 0 End ) AS VOID_AMOUNT, "
          ...
          "FROM ZEDS  "
         " INNER JOIN ( SELECT ZEDS.Z_KEY FROM ZEDS WHERE ZEDS.Time_Stamp >= :StartTime and ZEDS.Time_Stamp < :EndTime) zedcopy on zedcopy.Z_KEY=ZEDS.Z_KEY "
         " LEFT  JOIN ( "
                          "SELECT SUM(CAST(COALESCE(ARCBILL.TOTAL,0)  AS NUMERIC(17,4))) Refund_Amount, ARCBILL.Z_KEY FROM ARCBILL "
                           "INNER JOIN ZEDS ON ZEDS.Z_KEY=ARCBILL.Z_KEY "
                           "WHERE   (ZEDS.Time_Stamp >= :StartTime and ZEDS.Time_Stamp < :EndTime) and "
                           "ARCBILL.TOTAL < 0 GROUP BY ARCBILL.Z_KEY "

                    ")TOTALREFUNDS ON  zedcopy.Z_KEY = TOTALREFUNDS.z_key "


     " INNER JOIN "
        "("
                           "SELECT ARCBILL.ARCBILL_KEY,ARCBILL.Z_KEY,ARCBILL.SECURITY_REF from ZEDS "
                           "INNER JOIN ARCBILL on ARCBILL.Z_KEY = ZEDS.Z_KEY "
                           "WHERE  ZEDS.Time_Stamp >=:StartTime and ZEDS.Time_Stamp < :EndTime "
        ")ARCBILLNew on ARCBILLNew.Z_KEY= zedcopy.Z_KEY "  

         " INNER JOIN ( "
        
                            "SELECT ARCHIVE.ARCBILL_KEY,ARCHIVE.ARCHIVE_KEY ,ARCHIVE.PRICE ,ARCHIVE.SECURITY_REF from ZEDS "
                            "INNER JOIN ARCBILL on ARCBILL.Z_KEY = ZEDS.Z_KEY "
                            "INNER join ARCHIVE on ARCHIVE.ARCBILL_KEY = ARCBILL.ARCBILL_KEY "
                            "WHERE ZEDS.Time_Stamp >= :StartTime and ZEDS.Time_Stamp < :EndTime "
                    ") ARCHIVENew on ARCHIVENew.ARCBILL_KEY= ARCBILLNew.ARCBILL_KEY "

       "LEFT JOIN( "
                             "Select SUM(CAST(COALESCE(ARCHIVE.PRICE_LEVEL0,0) * ARCHIVE.QTY AS NUMERIC(17,4))) CANCEL_TOTAL, ARCHIVE.ARCBILL_KEY,CanSec.SECURITY_REF "
                             "FROM Security CanSec "
                             "Inner Join (select * from Archive where Archive.PRICE=0) Archive On "
                             "CanSec.Security_Ref = Archive.Security_Ref "
                             "Inner Join Contacts CanContact On "
                             "CanContact.Contacts_Key = CanSec.User_Key "
                             "Left Join ArcBill On "
                             ...
                             "CanSec.Time_Stamp >= :StartTime and "
                             "CanSec.Time_Stamp < :EndTime and "
                            "(CanSec.SECURITY_EVENT = 'Cancel' "
                            "OR CanSec.SECURITY_EVENT = 'CancelY' ) "
                            "and "
                            "OrdSec.Security_Event = 'Ordered By' and "
                            "Archive.Archive_Key Is Not Null and ARCHIVE.PRICE = 0 "
                            "group by Archive.ARCBILL_KEY,CanSec.SECURITY_REF "
                            ")CANCEL_AMOUNT on CANCEL_AMOUNT.SECURITY_REF = ARCHIVENew.SECURITY_REF "
         ...
         "LEFT JOIN ( "
                         "SELECT "
                              ...
                              /*  "MIN(CASE WHEN ARCORDERTAXES.TAX_TYPE = 2 THEN ARCORDERTAXES.TAX_VALUE END) AS ServiceCharge,      "
                                ...
                                "MIN(CASE WHEN ARCORDERTAXES.TAX_TYPE = 5 THEN ARCORDERTAXES.TAX_VALUE END) AS PROFITTAX "   */
                              "FROM ( "
                              "SELECT  a.ARCHIVE_KEY,a.TAX_TYPE,"
                              "Cast(Sum(a.TAX_VALUE ) as Numeric(17,4)) TAX_VALUE "
                              "FROM ARCORDERTAXES a "
                              "left join ( "
                              "SELECT ARCHIVE.ARCHIVE_KEY  from ZEDS "
                              "inner join ARCBILL on ARCBILL.Z_KEY = ZEDS.Z_KEY "
                              "inner join ARCHIVE on ARCHIVE.ARCBILL_KEY = ARCBILL.ARCBILL_KEY "
                              "WHERE   ZEDS.Time_Stamp >= :StartTime and ZEDS.Time_Stamp < :EndTime "
                    ") ARCHIVENew on ARCHIVENew.ARCHIVE_KEY= a.ARCHIVE_KEY "
                    ...
                    " ) "
                                "AOT ON ARCHIVENew.ARCHIVE_KEY = AOT.ARCHIVE_KEY "
          ...
          "LEFT JOIN ( "
                            ...
                            "FROM ARCHIVE DA "
                            "INNER join ARCBILL on ARCBILL.ARCBILL_KEY = DA.ARCBILL_KEY "
                            "LEFT JOIN ZEDS ON ZEDS.Z_KEY=ARCBILL.Z_KEY "
                        ...
                        ")  AND (UPPER(DA.DISCOUNT_REASON) NOT LIKE '%DIPLOMAT%') and  ZEDS.Time_Stamp >= :StartTime and ZEDS.Time_Stamp < :EndTime "
                         "group by da.ARCHIVE_KEY,da.DISCOUNT_WITHOUT_TAX,da.BASE_PRICE,da.QTY) VAT_EXEMPT_SALE ON ARCHIVENew.ARCHIVE_KEY = VAT_EXEMPT_SALE.ARCHIVE_KEY  "
                                           
           " inner JOIN  ( "
                                "SELECT discountsection.Z_KEY, "
                                ...
                                "CAST(CASE WHEN ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME = 'Senior Citizen' then sum(COALESCE(ARCORDERDISCOUNTS.DISCOUNTED_VALUE,0))end as NUMERIC(17,4)) SCD, "
                                "CAST(CASE WHEN ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME = 'Person with Disability' then sum(COALESCE(ARCORDERDISCOUNTS.DISCOUNTED_VALUE,0))end as NUMERIC(17,4)) PWD, "
                                "CAST(CASE WHEN ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME <> 'Person with Disability' AND ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME <> 'Senior Citizen' then sum(COALESCE(ARCORDERDISCOUNTS.DISCOUNTED_VALUE,0))end as NUMERIC(17,4)) discount "
                                "FROM ARCHIVE a "
                                "left JOIN ARCORDERDISCOUNTS on a.ARCHIVE_KEy = ARCORDERDISCOUNTS.ARCHIVE_KEY "
                                "left JOIN ARCBILL on a.ARCBILL_KEY = ARCBILL.ARCBILL_KEY "
                                "LEFT JOIN ZEDS ON ZEDS.Z_KEY=ARCBILL.Z_KEY "
                                "WHERE ZEDS.Time_Stamp >= :StartTime and ZEDS.Time_Stamp < :EndTime "
                                " GROUP BY ARCBILL.Z_KEY,ARCORDERDISCOUNTS.DISCOUNT_GROUPNAME) discountsection GROUP BY discountsection.Z_KEY "
                           " ) DISCOUNT_QUERY on ZEDS.Z_KEY = DISCOUNT_QUERY.Z_KEY  "

       
       "LEFT JOIN  ( "
                                "SELECT  a.ARCHIVE_KEY,sum(a.DISCOUNTED_VALUE) DISCOUNTED_VALUE,  a.DISCOUNT_GROUPNAME "
                                "FROM ARCORDERDISCOUNTS a "
                                "INNER JOIN ARCHIVE on a.ARCHIVE_KEY = archive.ARCHIVE_KEY "
                                "INNER JOIN ARCBILL on ARCHIVE.ARCBILL_KEY = ARCBILL.ARCBILL_KEY "
                                "LEFT JOIN ZEDS ON ZEDS.Z_KEY=ARCBILL.Z_KEY "
                                "WHERE ZEDS.Time_Stamp >= :StartTime  and ZEDS.Time_Stamp < :EndTime "
                                "GROUP BY a.ARCHIVE_KEY,a.DISCOUNT_GROUPNAME "
                    
                    ") ARCORDERDISCOUNTS on ARCHIVENew.ARCHIVE_KEY = ARCORDERDISCOUNTS.ARCHIVE_KEY "

        "LEFT join ( "
        
                                "SELECT DA.ARCHIVE_KEY, cast(coalesce(DA.BASE_PRICE,0)*da.QTY  + DA.DISCOUNT_WITHOUT_TAX + coalesce(AOT.TAX_VALUE,0) as numeric(17,4)) price "
                                "FROM ARCHIVE DA "
                                "LEFT JOIN (SELECT  a.ARCHIVE_KEY,Cast(Sum(coalesce(a.TAX_VALUE,0) ) as Numeric(17,4)) TAX_VALUE "
                                "FROM ARCORDERTAXES a "
                                ...
                                "ARCORDERTAXES.ARCHIVE_KEY NOT IN (SELECT a.ARCHIVE_KEY FROM ARCORDERDISCOUNTS a "
                                "WHERE (A.DISCOUNT_GROUPNAME = 'Senior Citizen' AND A.DISCOUNTED_VALUE <> 0) OR (A.DISCOUNT_GROUPNAME = 'Person with Disability'))) "

                        
                        ")zero_rated on ARCHIVENew.ARCHIVE_KEY = zero_rated.archive_key "

         " INNER join( "
                                "select cast(sum(coalesce(ARCBILLPAY.ROUNDING,0)) as numeric(17,4))rounding_amount,ARCBILL.Z_KEY from ARCBILLPAY "
                                "INNER JOIN ARCBILL on ARCBILLPAY.ARCBILL_KEY = ARCBILL.ARCBILL_KEY "
                                "LEFT JOIN ZEDS ON ZEDS.Z_KEY=ARCBILL.Z_KEY "
                                "WHERE  ZEDS.Time_Stamp >= :StartTime and ZEDS.Time_Stamp < :EndTime "
                                "group by ARCBILL.Z_KEY "

                    ")ROUNDING on zedcopy.Z_KEY = rounding.z_key "

        " INNER JOIN( "
                                "SELECT ARCBILL.INVOICE_NUMBER, ARCBILL.Z_KEY  FROM ARCBILL "
                                "right join (SELECT    MIN(ARCBILL.TIME_STAMP) min_ARCBILL_KEY  FROM ARCBILL "
                                "INNER JOIN ZEDS ON ZEDS.Z_KEY=ARCBILL.Z_KEY "
                                "WHERE   ZEDS.Time_Stamp >= :StartTime and ZEDS.Time_Stamp < :EndTime "
                                "GROUP BY ARCBILL.Z_KEY) ab on ARCBILL.TIME_STAMP=ab.min_ARCBILL_KEY "
       ") BEGINV ON BEGINV.Z_KEY = ARCBILLNew.Z_KEY "
       " INNER JOIN( "

                            "SELECT  ARCBILL.INVOICE_NUMBER, ARCBILL.Z_KEY  FROM ARCBILL "
                            "right join (SELECT    max(ARCBILL.TIME_STAMP) min_ARCBILL_KEY  FROM ARCBILL "
                            "INNER JOIN ZEDS ON ZEDS.Z_KEY=ARCBILL.Z_KEY "
                            "WHERE "
                            "ZEDS.Time_Stamp >= :StartTime and ZEDS.Time_Stamp < :EndTime "
                           " GROUP BY ARCBILL.Z_KEY) ab on ARCBILL.TIME_STAMP=ab.min_ARCBILL_KEY "
      ") ENDINV ON ENDINV.Z_KEY = ARCBILLNew.Z_KEY "
