...
void TdmMMReportData::SetupSalesSummaryD(TDateTime StartTime, TDateTime EndTime)
{
	 ...
	 qrSalesSummaryD->SQL->Text =
        ...
        "Select "
            ...
            "CAST(SUM(COALESCE(DISCOUNT_QUERY.PWD ,0)) AS NUMERIC(17,4)) PWD_Discount, "
            "CAST(SUM(COALESCE(DISCOUNT_QUERY.SCD ,0)) AS NUMERIC(17,4)) SCD_Discount, "
            "CAST(SUM(COALESCE(DISCOUNT_QUERY.SCD ,0)) + SUM(COALESCE(DISCOUNT_QUERY.PWD ,0)) + SUM(COALESCE(DISCOUNT_QUERY.discount ,0)) AS NUMERIC(17,4)) total_discount, "
          ...
          //      " - ((sum(coalesce(AOT.VAT,0)) + sum(coalesce(AOT.localTax,0))+ sum(coalesce(AOT.PROFITTAX,0)))) AS NUMERIC(17,4)) VATABLE,  "
            "CAST((ZEDS.TERMINAL_EARNINGS - sum(coalesce(VAT_EXEMPT_SALE.price,0)) - SUM(coalesce(zero_rated.price,0))) as NUMERIC(17,4)) VATABLE, "
            "CAST(sum(coalesce(AOT.VAT,0)) AS NUMERIC(17,4)) VAT, "
       ...
       "LEFT JOIN  (SELECT  a.ARCHIVE_KEY, "
                    ...
                    "FROM ARCORDERDISCOUNTS a "
                    "GROUP BY a.ARCHIVE_KEY ,a.DISCOUNT_GROUPNAME) DISCOUNT_QUERY on ARCHIVE.ARCHIVE_KEY = DISCOUNT_QUERY.ARCHIVE_KEY "
        "LEFT JOIN  (SELECT  a.ARCHIVE_KEY,sum(a.DISCOUNTED_VALUE) DISCOUNTED_VALUE,  a.DISCOUNT_GROUPNAME "
                    "FROM ARCORDERDISCOUNTS a "
                    "left join ARCORDERTAXES on a.ARCHIVE_KEY = ARCORDERTAXES.ARCHIVE_KEY   "
                    "WHERE  ARCORDERTAXES.TAX_TYPE = '0' AND ARCORDERTAXES.TAX_VALUE > 0    "
      ...
      /*  "LEFT JOIN "
                    "(select coalesce(q1.price,0) - coalesce(q2.tax_value,0) tax_exempt_SALE, q1.archive_key "
                    "from( "
                            "SELECT DA.ARCHIVE_KEY, coalesce(DA.PRICE,0) * DA.QTY price "
                            "                    FROM ARCHIVE DA "
                            "                    LEFT JOIN ARCORDERTAXES DAOT ON DAOT.ARCHIVE_KEY = DA.ARCHIVE_KEY "
                            "                    WHERE DA.ARCHIVE_KEY NOT IN ( "
                            "                        SELECT ARCHIVE_KEY "
                    ...
                    "     )) q1 "
        "left join (SELECT coalesce(TAX_VALUE,0) tax_value, ARCORDERTAXES.ARCHIVE_KEY "
                               "  FROM ARCORDERTAXES "
                               "  WHERE TAX_TYPE IN ('2','4') ) q2 on q1.archive_key = q2.archive_key) TAX_EXEMPT ON TAX_EXEMPT.ARCHIVE_KEY = ARCHIVE.ARCHIVE_KEY "  */
                               
        "left join (SELECT DA.ARCHIVE_KEY, coalesce(DA.PRICE,0) * DA.QTY price  "
                    "FROM ARCHIVE DA "
                    "LEFT JOIN ARCORDERTAXES DAOT ON DAOT.ARCHIVE_KEY = DA.ARCHIVE_KEY "
