...
"LEFT JOIN ( "		//Join with Tax
	...
	qrChronological->SQL->Text =	qrChronological->SQL->Text +
		...
		"Select "
			...
			"Orders.Qty, "
			"Orders.Size_Name, "
             "Cast((( CAST(Orders.QTY *  Orders.BASE_PRICE as Numeric(17,4)) ) +((cast(( (CAST(Orders.QTY * Orders.BASE_PRICE as Numeric(17,4))+ COALESCE(Orders.DISCOUNT_WITHOUT_TAX,0)))* CAST(COALESCE( AOT.VAT,0)/100 as Numeric(17,4))as         numeric(17, 4))  ) )+ "
             "( cast(( (CAST(Orders.QTY *  Orders.BASE_PRICE as Numeric(17,4)) + COALESCE( Orders.DISCOUNT_WITHOUT_TAX,0)))*CAST(COALESCE( AOT.ServiceCharge,0)/100 as Numeric(17,4)) as numeric(17, 4)) ) + "
            "(cast((((CAST(Orders.QTY *  Orders.BASE_PRICE as Numeric(17,4))+ COALESCE(Orders.DISCOUNT_WITHOUT_TAX,0)))*CAST(COALESCE(AOT.ServiceCharge,0)/100 as Numeric(17,4)))* CAST(COALESCE(STAX.ServiceChargeTax,0)/100 as Numeric(17,4)) as numeric(17, 4)))+  "
