...
void TdmMMReportData::SetupWagesByDepatment(TDateTime StartTime, TDateTime EndTime, TStrings *Names)
{
 ...
 "select "
           ...
           "from (select (ct.logout_datetime - ct.login_datetime) tt, "
                        ...
                        "C.Name, "
                        "C.Payroll_ID, "
                        "(SELECT ROUNDED_LOGIN_DATETIME FROM ROUNDEDCONTACTTIME WHERE LOGIN_DATETIME = ct.login_datetime  AND CONTACTTIME_KEY = ct.CONTACTTIME_KEY) Login_DateTime, "
                        "(SELECT ROUNDED_LOGOUT_DATETIME FROM ROUNDEDCONTACTTIME WHERE LOGIN_DATETIME = ct.login_datetime AND CONTACTTIME_KEY = ct.CONTACTTIME_KEY )  Logout_DateTime, "
                        "ct.Breaks, "
                        "ct.TOTALHOURS, "
                        "TCL.Name Department, "
                        "TCL.Code Zone, "
	...
	qrWages->SQL->Text =
        "select "
           ...
           "from (select (ct.logout_datetime - ct.login_datetime) tt, "
                        ...
                        "C.Name, "
                        "C.Payroll_ID, "
                        "cast((SELECT ROUNDED_LOGIN_DATETIME FROM ROUNDEDCONTACTTIME WHERE LOGIN_DATETIME = ct.login_datetime  AND CONTACTTIME_KEY = ct.CONTACTTIME_KEY)  as timestamp) Login_DateTime, "
                        "cast((SELECT ROUNDED_LOGOUT_DATETIME FROM ROUNDEDCONTACTTIME WHERE LOGIN_DATETIME = ct.login_datetime AND CONTACTTIME_KEY = ct.CONTACTTIME_KEY ) as timestamp) Logout_DateTime, "
                        "ct.Breaks, "
                        "ct.TOTALHOURS, "
                        "TCL.Name Department, "
                        "TCL.Code Zone, "
