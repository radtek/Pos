	...
	qrConsumption->SQL->Text		=	qrConsumption->SQL->Text +
		...
		"Select "
	        ...
	        "Cast((Orders.Qty * Orders.Cost) as Numeric(17,4)) Cost , "
          	"Cast(Null As VarChar(50)) Code, "
            "Cast((Orders.QTY * Orders.BASE_PRICE  + COALESCE(Orders.DISCOUNT_WITHOUT_TAX,0)+COALESCE(((Orders.Qty * Orders.BASE_PRICE+ Orders.DISCOUNT_WITHOUT_TAX))*Tax.VAT/100,0) "
            "+ COALESCE(((Orders.Qty * Orders.BASE_PRICE+ Orders.DISCOUNT_WITHOUT_TAX))*Tax.ServiceCharge/100,0)+ COALESCE((((Orders.Qty * Orders.BASE_PRICE+ Orders.DISCOUNT_WITHOUT_TAX))*Tax.ServiceCharge/100)*STAX.ServiceChargeTax/100,0)) as Numeric(17,4)) SalesIncl "
