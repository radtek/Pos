	...
	qrStockVariance->SQL->Text =
		...
		"From "
			...
			"Left Join StockLocation On "
				"Stock.Stock_Key = StockLocation.Stock_Key "
               " left join(select STOCKTRANS.CODE, sum(stocktrans.QTY) QTY, STOCKTRANS.LOCATION from STOCKTRANS where stocktrans.TRANSACTION_TYPE='Manufacture'  and STOCKTRANS.CREATED>="
