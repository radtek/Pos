	...
	TStrings *Locations, TStrings *Groups)
{
	...
	qrStockMovement->SQL->Text =
		"Select "
            ...
            "StockTrans.Note,"
			"cast(Sum(StockTrans.Unit_Cost * StockTrans.Qty) as float) Total_Cost "
		"From "
			"StockTrans "
		"Where "
...
void TdmStockReportData::SetupStockInquiry (TDateTime StartTime, TDateTime EndTime, TStrings *Locations, TStrings *Groups, TStrings *Transactions)
{
	...
	qrStockInquiry->SQL->Text =
		"Select "
			...
			"StockTrans.Unit,"
			"StockTrans.Qty,"
			"cast(StockTrans.Unit_Cost * StockTrans.Qty as float) Total_Cost,"
...
void TdmStockReportData::SetupStockVariance(int StocktakeKey,int RadioButtonValue,AnsiString VarianceAmount)
{
	...
	qrStockVariance->SQL->Text =
		...
		"From "
			...
			"Left Join StockLocation On "
				"Stock.Stock_Key = StockLocation.Stock_Key "
               " left join(select STOCKTRANS.CODE,stocktrans.QTY,STOCKTRANS.LOCATION from STOCKTRANS where stocktrans.TRANSACTION_TYPE='Manufacture'  and STOCKTRANS.CREATED>="
			"(select max(STOCKTAKEHISTORY.LAST_STOCKTAKE)from STOCKTAKEHISTORY ))STK on STOCK.CODE=STK.CODE and STK.LOCATION=StockLocation.LOCATION "
		"Where "
			"Stock.Enabled = 'T' And "
			"(Stock.Deleted = 'F' Or Stock.Deleted Is Null) And "
	...
	"		Inner Join StockLocation On "
	"			Stock.Stock_Key = StockLocation.Stock_Key "
    "        Left join (select sum(stocktrans.QTY)QTY,STOCKTRANS.LOCATION,STOCKTRANS.STOCK_GROUP from STOCKTRANS where stocktrans.TRANSACTION_TYPE='Manufacture'  and STOCKTRANS.CREATED>=  "
...
{         int prevStocktakeKey =StocktakeKey-1;
	...
	qrStockVariance->SQL->Text =
		"Select "
			...
			"StocktakeHistory.Sales + StocktakeHistory.Transfer + StocktakeHistory.Writeoff + StocktakeHistory.Inwards + StocktakeHistory.Opening  On_Hand,"
		...
		"From "
			"StocktakeHistory left join(select STOCKTRANS.CODE,stocktrans.QTY,STOCKTRANS.LOCATION from STOCKTRANS where stocktrans.TRANSACTION_TYPE='Manufacture'  and STOCKTRANS.CREATED>="
			"(select max(STOCKTAKEHISTORY.LAST_STOCKTAKE)from STOCKTAKEHISTORY /*where STOCKTAKEHISTORY.STOCKTAKE_KEY = (select max(STOCKTAKEHISTORY.STOCKTAKE_KEY) from STOCKTAKEHISTORY)*/))STK on STOCKTAKEHISTORY.CODE=STK.CODE and STK.LOCATION=STOCKTAKEHISTORY.LOCATION "
		"Where "
   ...
   "(Select  "
		...
		"From "
			"StocktakeHistory Left Join Stock On  "
			 "StocktakeHistory.Code = Stock.Code left join (select sum(stocktrans.QTY)QTY,STOCKTRANS.LOCATION,STOCKTRANS.STOCK_GROUP from STOCKTRANS where stocktrans.TRANSACTION_TYPE='Manufacture'  and STOCKTRANS.CREATED>= "
