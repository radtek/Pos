...
{
//---------------------------------------------------------------------------
enum TTransactionType { ttTransfer, ttReceipt, ttSale, ttWriteoff, ttStocktake, ttManufacture, ttPackingSlip };
//---------------------------------------------------------------------------
...
public:
	TTransactionInfo(TTransactionType iTransactionType, TTransactionBatchInfo const& iBatchInfo) :
		Qty(0), Supplier_Key(0), Order_Qty(0), Unit_Cost(0), GST_Percent(0), Total_GST(0), Note("")
    ...
    AnsiString                  Operator;
    AnsiString                  Note;
    AnsiString	Supplier_Unit;
    int TransactionNumber;
    AnsiString			Reference;		// Invoice Number
   // int Transfer_Id;
    int Stock_Request_number;
    int Stock_Request_Stock_Key_Value;
    int Transfer_id;
    double REQUESTED_QTY_ORIGINAL_VAL;    
...
public:
    ...
    int    STOCK_REQUEST_NO;
    int   STOCK_REQUEST_STOCK_KEY;
    int Transfer_id;
    double REQUESTED_QTY_ORIGINAL_VAL;
...
protected:
    ...
    void				fUpdateBatchTransaction(TTransactionInfo const& TransactionInfo);
...
private:
    ...
    bool						fReceivePackingSlipItem(TTransactionBatchInfo const& BatchInfo, TiInvoiceItemInfo const &InvoiceItemInfo);
...
public:
    ...
    bool RecievePackingSlip(TTransactionBatchInfo& BatchInfo, TcInvoiceItemInfo const& InvoiceItems);
};
//---------------------------------------------------------------------------
...
public:
	...
	bool	AdjustStock(TSaleItemInfo& SaleItemInfo, AnsiString Operator);
	bool	Close(bool Commit);
};
//---------------------------------------------------------------------------
class TWriteOffStock : public TStockTransaction
{
private:
	Database::TcpIBSQL		sqlSelectStock;
	Database::TcpIBSQL		sqlSelectGroup;
	Database::TcpIBSQL		sqlSelectCategory;
	Database::TcpIBSQL		sqlSelectLocation;
...
public:
    TWriteOffStock(TIBDatabase *IBDatabase);
    void UpdateStock(AnsiString temp[], double Qty, TDateTime Date);
};
//---------------------------------------------------------------------------
class TManufactureStock : public TStockTransaction
{
private:
	Database::TcpIBSQL		sqlSelectStock;
	Database::TcpIBSQL		sqlSelectGroup;
