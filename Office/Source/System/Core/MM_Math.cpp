...
//---------------------------------------------------------------------------
Currency MMMath::RoundCurrencyTo10c(const Currency Value)
{
	if (Value > 0)	return Currency(int(Value * 10 + 0.5)) / 10;
	else				return Currency(int(Value * 10 - 0.5)) / 10;
}
//---------------------------------------------------------------------------
...
/*		int PrecisionUse = Precision;
		...
		if (PrecisionUse > Decimals + 2)
		{
			PrecisionUse = Decimals + 2;
		}*/

		int PrecisionUse = Decimals + 2;

		unsigned __int64 SignificantDigits = 0;

		long double PrecisionMul = 1;
		for (int j=0; j<PrecisionUse; j++, PrecisionMul *= 10);

		try
		{
			long double temp = (long double)Value * PrecisionMul + (long double)(0.1);
		...
		if (Value > 0)
		{
			return double((unsigned __int64)(((long double)((unsigned __int64)(SignificantDigits / Div)) + (long double)(0.5)))) / (long double)(Mul);
		}
		else
		{
