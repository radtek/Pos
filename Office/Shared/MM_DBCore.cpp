					...
					Port				(iPort)
{
	...
	{
		int Pos = iFileName.AnsiPos(":\\");
		if (Pos != 0 && Pos != 2)
		{
			AnsiString File;
			int ServerPos	= iFileName.AnsiPos(":");
...
bool TDBControl::WaitForIBServer(unsigned int Timeout)
{
	...
	do
	{
		Found = FindWindow("IB_Server", "InterBase Server");
		if (Found == false)
		{
			Sleep(500);
		}
...
//---------------------------------------------------------------------------
void TDBUtilities::DropDatabase()
{
	if (!fDBControl.fIBDatabase->Connected)
	{
		fDBControl.Connect();
	}
	fDBControl.fIBDatabase->DropDatabase();
}
//---------------------------------------------------------------------------
