...
typedef void __fastcall (__closure *TEventNewsgroupList)(const AnsiString ANewsgroup, const int ALow, const int AHigh, const AnsiString AType, bool &ACanContinue);
...
public:
	...
	void __fastcall ParseXOVER(AnsiString Aline, int &AArticleIndex, AnsiString &ASubject, AnsiString &AFrom, System::TDateTime &ADate, AnsiString &AMsgId, AnsiString &AReferences, int &AByteCount, int &ALineCount, AnsiString &AExtraData);
	void __fastcall ParseNewsGroup(AnsiString ALine, AnsiString &ANewsGroup, int &AHi, int &ALo, AnsiString &AStatus);
...
public:
	#pragma option push -w-inl
	/* EIdProtocolReplyError.CreateError */ inline __fastcall virtual EIdNNTPConnectionRefused(const int AErrCode, const AnsiString AReplyMessage) : Idexception::EIdProtocolReplyError(AErrCode, AReplyMessage) { }
	#pragma option pop
