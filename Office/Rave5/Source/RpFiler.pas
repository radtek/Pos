  ...
  begin { WriteFont }
    ...
    with BaseFont do begin
      ...
      if (Charset <> SaveFont.Charset) or WriteAll then begin
        ...
        WriteByte((TFontCharset(Charset)));
...
{ Property Get/Set routines }
  ...
  begin { SetBKColor }
    ...
    if Printing then begin
      ...
      WriteLongint(longint(Windows.GetBKColor(Canvas.Handle)));
    end; { if }
  end;  { SetBKColor }
...
{$IFDEF LEVEL6}{$WARN SYMBOL_DEPRECATED OFF}{$ENDIF}
{$IFDEF LEVEL6}{$WARN SYMBOL_LIBRARY OFF}{$ENDIF}
  procedure TRvNDRWriter.BrushCopy(const Dest: TRect;
                                         Bitmap: TBitmap;
                                   const Source: TRect;
                                         Color: TColor);
  begin { BrushCopy }
  ...
  procedure TRvNDRWriter.FloodFill(X,Y: double;
                                   Color: TColor;
  ...
  procedure TRvNDRWriter.FrameRect(const Rect: TRect);
  begin { FrameRect }
    CheckPrinting;
    if OutputInvalid then Exit;
