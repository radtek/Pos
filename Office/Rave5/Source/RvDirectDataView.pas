...
uses
  Windows,
  Classes, RpCon, RvData, RvDataField;
...
uses
  ...
  RpDefine, RvClass, RvDefine, RvProj,
  RvUtil, SysUtils, TypInfo;
...
{$ENDIF}
  ...
  if CurrentFileMap > 0 then begin
    RpDefine.CloseFileMap(AltFileMap,AltFileBuf);
  end; { if }
  RpDefine.CloseFileMap(NormFileMap,NormFileBuf);
...
var
  ...
  begin { ConnectRemote }
    ...
    DataCon.AppHandle := HWND(ReadInt);
