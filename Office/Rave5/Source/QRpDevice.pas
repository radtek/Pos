...
uses
  ...
  QGraphics, QDialogs, QForms, Types, Qt,
  SysUtils, Classes, QRpDefine;
...
type
  ...
  //!!PORT!! #2 RPDEVICE01 These options have to be adjusted for QPrintDialog
...
begin
  inherited Create;
  FCopies := 1;
  FOrientation := QRpDefine.poLandscape;
...
//  GetTextExtentPoint32({$IFDEF RaveCLX}QPainter_handle{$ENDIF}(Canvas.Handle),@Value[1],Length(Value),TextExtent);
  //Result := Canvas.TextWidth(Value);
    Canvas.Start;
    GetTextExtentPoint32(QPainter_Handle(Canvas.Handle),@Value[1],Length(Value),TextExtent);
    Canvas.Stop;
...
begin
  ...
  try
    ...
    //!!PORT!! RPDEVICE08 #46 - for some reason, the Escape call doesn't compile in CLX
  finally
...
//!!PORT!! - Can't assign a Windows handle to QPaintDevice
        end; { if }
      end; { with }
    end; { if }
    FDeviceState := Value;
