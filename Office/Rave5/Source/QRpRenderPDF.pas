...
type
  ...
  public
    ...
    procedure PrintBitmapRect(const X1, Y1, X2, Y2: double; AGraphic: QGraphics.TBitmap); override;
    procedure PrintBitmap(const X1,Y1,ScaleX,ScaleY: double; AGraphic: QGraphics.TBitmap); override;
    ...
    function AddJPG(const AImage: TNDGraphic; AReuseJPG: boolean): string;
    procedure PrintJPG(const AX1, AY1, AX2, AY2: double; AName: string);
...
begin
  ...
  FFontBitmap := QGraphics.TBitmap.Create;
...
function TRvRenderPDF.AddJPG(const AImage: TNDGraphic;
...
begin
  case ConvertCharset(CLXToVCLCharSet(Converter.Font.Charset)) of
...
begin
  ...
  GetTextMetrics(QPainter_handle(FontBitmap.Canvas.Handle),TextMetric);
...
begin
  ...
  nSize := GetOutlineTextMetrics(QPainter_handle(RvRenderPDF.FontBitmap.Canvas.Handle),
                      sizeof(OutlineTextMetric),
                      nil);
  GetMem(OutlineTextMetric, nSize);
  try
    if GetOutlineTextMetrics(QPainter_handle(RvRenderPDF.FontBitmap.Canvas.Handle),
                        nSize,
      ...
      if GetCharABCWidths(QPainter_handle(RvRenderPDF.FontBitmap.Canvas.Handle),
                          FirstChar,
                          LastChar,
                          lpABC) then begin
        SetWidths;
    ...
    if RvRenderPDF.EmbedFonts and IsEmbeddingAllowed then begin
      ...
      FontFile.LoadFontFile(QPainter_handle(RvRenderPDF.FontBitmap.Canvas.Handle));
    end; { if }
  finally
...
begin
  ...
  GetTextExtentPoint32(QPainter_handle(RvRenderPDF.FontBitmap.Canvas.Handle),
