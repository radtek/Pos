...
uses
  Qt, QTypes, QGraphics, QControls, QForms, QDialogs, QStdCtrls,
  QButtons, QExtCtrls, QMenus, QActnList, QImgList, QComCtrls,
  SysUtils, Classes, QRpBase, QRpFiler, QRpRenderPrinter, QRpRenderPreview,
  QRpDefine, QRpSystem, QRpFormSetup;
    ...
    try
      ...
      try
        ...
        if dlogSave.FilterIndex > 1 then begin // Do Render
          ...
          try
            QRpRender.GetRenderList(LRenderList);
...
begin
  ...
  if QRpRender.GetRenderFilter <> '' then begin
    dlogSave.Filter := dlogSave.Filter + '|' + QRpRender.GetRenderFilter;
  end; { if }
end;
