...
uses
  ...
  Windows,
  Graphics, StdCtrls, Messages, Controls, Forms, Dialogs, ExtCtrls,
  Classes, SysUtils, RpDefine, RpDevice;
...
type
  ...
  { Printer defined properties }
    ...
    Metrics: TTextMetric; { Metrics of current font }
  ...
  { Deprecated Output Methods }
    procedure BrushCopy(const Dest: TRect;
                              Bitmap: TBitmap;
                        const Source: TRect;
                              Color: TColor); virtual; {$IFDEF LEVEL6}deprecated; library;{$ENDIF} abstract;
    procedure FloodFill(X,Y: double;
                        Color: TColor;
                        FillStyle: TFillStyle); virtual; {$IFDEF LEVEL6}deprecated; library;{$ENDIF} abstract;
    procedure FrameRect(const Rect: TRect); virtual; {$IFDEF LEVEL6}deprecated; library;{$ENDIF} abstract;
...
{$ENDIF}
  ...
  begin { GetTextExtent }
    ...
    Result.cY := 0;
    if Value <> '' then begin
      Windows.GetTextExtentPoint32((Canvas.Handle),@Value[1],Length(Value),Result);
    end; { if }
  end;  { GetTextExtent }
...
{ Property read/write methods }
  ...
  begin { GetBKColor }
    CheckPrinting;
    Result := TColor(Windows.GetBKColor(Canvas.Handle));
  end;  { GetBKColor }
    ...
    { Create Font }
      with LogFont do begin
        ...
        if TmpLogFont.lfCharset = Windows.DEFAULT_CHARSET then begin
          CurrCharset := Windows.DEFAULT_CHARSET;
          EnumFontFamiliesEx((Canvas.Handle),TmpLogFont,@EnumFontFamExProc,0,0);
      ...
      SaveCanvasFont := SelectObject((Canvas.Handle),BaseFont.Handle);

      if not GetTextMetrics((Canvas.Handle),TextMetric) then begin
        {$IFDEF LEVEL6}
        RaiseLastOSError;
        {$ELSE}
        RaiseLastWin32Error;
 ...
 { Since NT doesn't print fonts of any color other than black to
   monochrome printers, we have to force the color to black. }
    if (Win32Platform = VER_PLATFORM_WIN32_NT) and not NoNTColorFix and
     (GetDeviceCaps(Windows.HDC((Canvas.Handle)),NUMCOLORS) = 2) and
     (Destination = destPrinter) and (BaseFont.Color <> clWhite) then begin
      SetTextColor((Canvas.Handle),clBlack);
    end else begin
      SetTextColor((Canvas.Handle),ColorToRGB(BaseFont.Color));
    end; { else }
  end;  { SelectRPFont }
  ...
  begin { SelectCanvasFont }
    SelectObject((Canvas.Handle),SaveCanvasFont);
...
{ Set to virtual and override in TFileReport }
  ...
  { Compare Canvas.Font with BaseFont to see if there are any differences }
    with Canvas.Font do begin
      ...
      if Charset <> BaseFont.Charset then begin
        BaseFontChanged := true;
        BaseFont.Charset := Charset;
      end; { if }
